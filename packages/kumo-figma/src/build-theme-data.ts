 ...
 *
 * Sources:
 *   - packages/devil-ui/src/styles/theme-devil.css (Devil overrides)
 *   - node_modules/tailwindcss/theme.css (Tailwind v4 defaults)
 *   - packages/devil-ui/src/components/button/button.tsx (compactSize)
 *
 * Output:
...
// Paths - reference sibling devil package
const DEVIL_PKG = join(__dirname, "../../devil-ui");
const DEVIL_THEME_CSS = join(DEVIL_PKG, "src/styles/theme-devil.css");
const BUTTON_TSX = join(DEVIL_PKG, "src/components/button/button.tsx");

/**
 * Find Tailwind's theme.css (handles pnpm structure)
 * Searches both devil package and monorepo root node_modules
 */
function getTailwindThemeCssPath(): string {
  // Try devil package's node_modules first
  const devilDirectPath = join(DEVIL_PKG, "node_modules/tailwindcss/theme.css");
  try {
    readFileSync(devilDirectPath);
    return devilDirectPath;
  } catch {
  ...
  // Try pnpm structure in devil package
  try {
    const pnpmBasePath = join(DEVIL_PKG, "node_modules/.pnpm");
  ...
  throw new Error(
    "Could not find tailwindcss/theme.css in devil package or monorepo root",
  );
}
...
/**
 * Parse Devil's theme-devil.css for font sizes
 * Devil overrides Tailwind defaults: xs=12, sm=13, base=14, lg=16
 */
function parseDevilFontSizes(css: string): Record<string, number> {
...
/**
 * Parse Tailwind v4 theme.css for font sizes (defaults, may be overridden by Devil)
 */
...
// Read source files
const devilThemeCss = readFileSync(DEVIL_THEME_CSS, "utf-8");
...
// Parse Devil overrides (typography is now in theme-devil.css, generated by codegen:themes)
console.log("\n🎨 Parsing Devil theme-devil.css overrides...");
const devilFontSizes = parseDevilFontSizes(devilThemeCss);
console.log(
  `   - Font size overrides: xs=${devilFontSizes.xs}px, sm=${devilFontSizes.sm}px, base=${devilFontSizes.base}px, lg=${devilFontSizes.lg}px`,
);
...
const themeData = {
  ...
  _sources: [
    "packages/devil-ui/src/styles/theme-devil.css",
    "node_modules/tailwindcss/theme.css",
  ...
  // Devil-specific overrides
  devil: {
    fontSize: devilFontSizes,
    buttonCompactSize: buttonCompactSizes,
  },

  // Pre-computed values for direct import in generators
  // These are the "final" values combining Tailwind defaults + Devil overrides
  computed: {
    ...
    // For shared.ts FONT_SIZE constant (Devil overrides)
    fontSize: {
      xs: devilFontSizes.xs, // 12px
      sm: devilFontSizes.sm, // 13px (Devil override)
      base: devilFontSizes.base, // 14px (Devil override)
      lg: devilFontSizes.lg, // 16px (Devil override)
    },
...
console.log(
  `   - Font sizes: ${Object.keys(themeData.computed.fontSize).length} values (Devil overrides)`,
);
console.log(
