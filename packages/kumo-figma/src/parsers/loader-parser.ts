...
export type LoaderData = {
  ...
  /** Circle elements */
  circles: CircleData[];
  /** Size variants from DEVIL_LOADER_VARIANTS */
...
export function parseLoaderSvg(): LoaderData {
  // Resolve loader.tsx path - reference sibling devil package
  const __filename = fileURLToPath(import.meta.url);
  const __dirname = dirname(__filename);
  const loaderPath = join(
    __dirname,
    "../../../devil-ui/src/components/loader/loader.tsx",
  );

  const content = readFileSync(loaderPath, "utf-8");

  // Extract DEVIL_LOADER_VARIANTS
  const sizesMatch = content.match(
    /DEVIL_LOADER_VARIANTS\s*=\s*\{[\s\S]*?size:\s*\{([\s\S]*?)\}\s*,?\s*\}/,
  );
