 ...
 * Searches both local package, devil package, and monorepo root
 */
export function getTailwindThemeCssPath(): string {
  ...
  const searchPaths = [
    ...
    process.cwd(),
    // Devil package (sibling)
    join(process.cwd(), "../devil-ui"),
  ...
  throw new Error(
    "Could not find tailwindcss/theme.css in node_modules. " +
      "Ensure tailwindcss is installed in devil package or monorepo root.",
  );
}
