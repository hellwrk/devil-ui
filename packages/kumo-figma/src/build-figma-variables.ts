 ...
 *
 * Source:
 *   packages/devil-ui/scripts/theme-generator/config.ts
 *
 * Output:
...
import { THEME_CONFIG } from "@hellwrk/devil-ui/scripts/theme-generator/config";
...
function main() {
  ...
  for (const [tokenName, def] of Object.entries(THEME_CONFIG.text)) {
    const devilTheme = def.theme.devil;
    if (!devilTheme) continue;

    colorVariables.push({
      name: `text-color-${tokenName}`,
      light: parseColorToRgb(devilTheme.light),
      dark: parseColorToRgb(devilTheme.dark),
    });
  }
  ...
  const output = {
    ...
    _source: "packages/devil-ui/scripts/theme-generator/config.ts",
