 ...
 *
 * Generates Figma components from Devil component definitions.
 * Runs destructive sync - purges and recreates all components on each run.
 *
 * Target file: sKKZc6pC6W1TtzWBLxDGSU (devil-ai)
 */
...
figma.ui.onmessage = async (msg: { type: string }) => {
  if (msg.type === "generate") {
    try {
      figma.notify("Starting Devil UI Kit generation...");
      ...
      if (message.includes("kumo-colors") || message.includes("collection")) {
        ...
        logError(
          "Devil semantic color tokens not found. Please sync tokens from CSS to Figma variables.",
        );
      }
