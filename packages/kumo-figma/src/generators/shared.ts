 ...
 * which get translated to the actual "devil-*" variable names.
 */
export const VAR_NAMES = {
  // Text colors
  text: {
    default: "text-color-devil-default",
    inverse: "text-color-devil-inverse",
    strong: "text-color-devil-strong",
    subtle: "text-color-devil-subtle",
    inactive: "text-color-devil-inactive",
    brand: "text-color-devil-brand",
    link: "text-color-devil-link",
    success: "text-color-devil-success",
    danger: "text-color-devil-danger",
    warning: "text-color-devil-warning",
    // Legacy aliases (for gradual migration)
    surface: "text-color-devil-default",
    muted: "text-color-devil-subtle",
    label: "text-color-devil-strong",
    error: "text-color-devil-danger",
    disabled: "text-color-devil-inactive",
  },
  // Background/fill colors
  color: {
    base: "color-devil-base",
    elevated: "color-devil-elevated",
    recessed: "color-devil-recessed",
    overlay: "color-devil-overlay",
    contrast: "color-devil-contrast",
    control: "color-devil-control",
    tint: "color-devil-tint",
    interact: "color-devil-interact",
    fill: "color-devil-fill",
    fillHover: "color-devil-fill-hover",
    brand: "color-devil-brand",
    brandHover: "color-devil-brand-hover",
    line: "color-devil-line",
    ring: "color-devil-hairline",
    arrowEdge: "color-devil-arrow-edge",
    arrowStroke: "color-devil-arrow-stroke",
    info: "color-devil-info",
    infoTint: "color-devil-info-tint",
    warning: "color-devil-warning",
    warningTint: "color-devil-warning-tint",
    danger: "color-devil-danger",
    dangerTint: "color-devil-danger-tint",
    success: "color-devil-success",
    successTint: "color-devil-success-tint",
    // Legacy aliases (for gradual migration from old naming convention)
    // Old: "color-{name}" → New: "color-devil-{name}"
    surface: "color-devil-base",
    surfaceInverse: "color-devil-contrast",
    secondary: "color-devil-control",
    primary: "color-devil-brand",
    border: "color-devil-line",
    active: "color-devil-brand",
    hover: "color-devil-interact",
    error: "color-devil-danger",
    accent: "color-devil-tint",
  },
} as const;
...
/**
 * Font size constants
 * Generated from: theme-devil.css --text-* values (Devil overrides Tailwind defaults)
 */
export const FONT_SIZE = {
  /** Extra small - matches --text-xs in theme-devil.css */
  xs: themeData.computed.fontSize.xs,
  /** Small - matches --text-sm in theme-devil.css (Devil override) */
  sm: themeData.computed.fontSize.sm,
  /** Base - matches --text-base in theme-devil.css (Devil override) */
  base: themeData.computed.fontSize.base,
  /** Large - matches --text-lg in theme-devil.css (Devil override) */
...
export const FALLBACK_VALUES = {
  /** Default font size (text-base from theme-devil.css) */
   ...
   * Generated from: button.tsx DEVIL_BUTTON_VARIANTS.compactSize
   */
  buttonCompactSize: {
 ...
 * e.g., "color-devil-info/20" → { baseName: "color-devil-info", opacity: 0.2 }
 * e.g., "color-devil-brand" → { baseName: "color-devil-brand", opacity: undefined }
 */
 ...
 * Handles patterns like "color-devil-info/20" by creating a new variable with
 ...
 *
 * @example
 * bindFillToVariableWithOpacity(frame, "color-devil-info/20"); // Creates/uses color-devil-info/20 variable
 * bindFillToVariableWithOpacity(frame, "color-devil-brand"); // Uses color-devil-brand directly
 */
...
let _devilColorsCollection: VariableCollection | null = null;
...
export function initializeColorVariables(): VariableCollection {
  ...
  if (_devilColorsCollection) {
    return _devilColorsCollection;
  }

  const collections = figma.variables.getLocalVariableCollections();
  let devilColors: VariableCollection | undefined = collections.find(
  ...
  if (!devilColors) {
    ...
    devilColors = figma.variables.createVariableCollection(
      figmaVariablesData.collectionName,
    );

    // Rename default mode to "Light"
    devilColors.renameMode(devilColors.modes[0].modeId, "Light");

    // Add Dark mode
    devilColors.addMode("Dark");
  }

  _devilColorsCollection = devilColors;

  // Get mode IDs
  const lightModeId = devilColors.modes.find((m) => m.name === "Light")?.modeId;
  const darkModeId = devilColors.modes.find((m) => m.name === "Dark")?.modeId;
  ...
  const existingVars = devilColors.variableIds
...
export function clearColorVariables(): void {
  const collections = figma.variables.getLocalVariableCollections();
  const devilColors = collections.find(
    (c) => c.name === figmaVariablesData.collectionName,
  );

  if (devilColors) {
    // Remove all variables
    for (const varId of devilColors.variableIds) {
 ...
 *
 * @param variableName - Variable name (e.g., "text-color-devil-default", "color-devil-base")
 * @returns Variable or undefined if not found
 *
 * @example
 * const textVar = getVariableByName("text-color-devil-default");
 * if (textVar) {
...
export function getVariableByName(variableName: string): Variable | undefined {
  ...
  const variables = devilColors.variableIds
    .map((id) => figma.variables.getVariableById(id))
    .filter((v): v is Variable => v !== null);
 ...
 * If the variable name includes an opacity suffix (e.g., "color-devil-info/20"),
 * this will create a new variable with the opacity pre-multiplied into the RGB values.
 *
 * @param variableNameWithOpacity - Variable name possibly with opacity suffix (e.g., "color-devil-info/20")
...
export function getOrCreateVariableWithOpacity(
  ...
  // Dark mode: blend towards devil-base dark which is black (0, 0, 0)
  //
 ...
 */
function getDevilColorsModes(): {
  collection: VariableCollection;
  lightModeId: string;
  darkModeId: string;
} | null {
  const collections = figma.variables.getLocalVariableCollections();
  const devilColors = collections.find((c) => c.name === "kumo-colors");
  ...
  const lightMode = devilColors.modes.find(
    (m) => m.name.toLowerCase() === "light",
  );
  const darkMode = devilColors.modes.find(
...
/**
 * Create a frame with bg-devil-base variable fill and explicit color mode
 *
 * The frame provides:
 * - Variable-bound background (bg-devil-base)
...
export function createModeSection(
  ...
  const modesInfo = getDevilColorsModes();
  if (modesInfo) {
    const modeId =
...
export async function createComponentSectionPair(
  ...
  // Bind title color to text-color-devil-default variable
