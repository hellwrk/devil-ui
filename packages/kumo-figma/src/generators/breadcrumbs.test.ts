...
describe("Breadcrumbs Generator - Color Bindings", () => {
  ...
  it("should use semantic devil color tokens", () => {
    const bindings = getBreadcrumbsColorBindings();
    // Verify tokens follow the devil semantic pattern without hardcoding exact values
    expect(bindings.link).toMatch(/^text-color-devil-/);
    expect(bindings.current).toMatch(/^text-color-devil-/);
    expect(bindings.separator).toMatch(/^text-color-devil-/);
  });
