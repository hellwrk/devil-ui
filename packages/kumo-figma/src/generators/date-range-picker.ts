 ...
 *
 * @see packages/devil-ui/src/components/date-range-picker/date-range-picker.tsx
 */
...
> = {
  sm: {
    ...
    cellHeight: 22,
    cellWidth: 24,
    textSize: FONT_SIZE.xs, // 12px from theme-devil.css
  ...
  base: {
    ...
    cellHeight: 26,
    cellWidth: 28,
    textSize: FONT_SIZE.base, // 14px from theme-devil.css
  ...
  lg: {
    ...
    cellHeight: 32,
    cellWidth: 36,
    textSize: FONT_SIZE.lg, // 16px from theme-devil.css
...
function getVariantBackground(variant: string): string {
  ...
  if (classes.indexOf("bg-devil-overlay") >= 0) {
    return VAR_NAMES.color.overlay;
  } else if (classes.indexOf("bg-devil-base") >= 0) {
...
async function createMonthHeader(
  ...
  if (showLeftNav) {
    ...
    bindIconColor(leftIcon, "text-devil-default");
  ...
  if (showRightNav) {
    ...
    bindIconColor(rightIcon, "text-devil-default");
...
async function createFooter(
  ...
  bindIconColor(globeIcon, "text-devil-subtle");
...
export function getDateRangePickerSizeConfig() {
  return {
    ...
    config: SIZE_CONFIG, // hardcoded (TODO: extract from DEVIL_DATE_RANGE_PICKER_VARIANTS)
