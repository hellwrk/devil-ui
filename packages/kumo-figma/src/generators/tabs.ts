 ...
 *
 * Structure:
 * - Horizontal container with rounded background (bg-devil-tint)
 * - Active tab has elevated white pill indicator (bg-devil-control with shadow)
 * - Inactive tabs show muted text
 * - Active tab has dark text
 *
 * @see packages/devil-ui/src/components/tabs/tabs.tsx
 * @see packages/devil-ui/src/components/tabs/tabs.stories.tsx
 */
 ...
 * - Indicator: rounded-lg, bg-secondary, shadow-sm, ring ring-devil-fill-hover
 *
...
const FALLBACK_TABS_CONFIG = {
  ...
  /** Tab text size (text-base from Tailwind = 16px, or Devil = 14px) */
...
async function createTabButton(
  ...
  // Text color: text-devil-default for active, text-devil-strong for inactive
  if (isActive) {
...
function createTabIndicator(
  ...
  // Styling: rounded-lg, bg-devil-control
  ...
  // Border: ring ring-devil-fill-hover (1px)
...
async function createTabsComponent(
  ...
  // Styling: rounded-lg, bg-devil-tint
  component.cornerRadius = TABS_CONFIG.borderRadius;

  // Background: bg-devil-tint
