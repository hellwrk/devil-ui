...
describe("Loader Generator - Color Binding", () => {
  ...
  it("should use devil semantic token for stroke", () => {
    const colorBinding = getLoaderColorBinding();
    // Stroke should use a devil text color token for theming support
    expect(colorBinding.strokeVariable).toMatch(/^text-color-devil-/);
  });
});
