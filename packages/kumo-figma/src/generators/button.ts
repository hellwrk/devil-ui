 ...
 *
 * @see packages/devil-ui/src/components/button/button.tsx
 */
...
/**
 * Compact size mapping from button.tsx DEVIL_BUTTON_VARIANTS.compactSize
 * Used for square and circle shapes.
 * Parses size-* Tailwind classes to derive pixel values.
 *
 * Source: button.tsx DEVIL_BUTTON_VARIANTS.compactSize
 * - xs: size-3.5 = 14px
 * - sm: size-6.5 = 26px
 * - base: size-9 = 36px
 * - lg: size-10 = 40px
...
export function getCompactSizeMap(): Record<string, number> {
  // Compact size classes from DEVIL_BUTTON_VARIANTS.compactSize in button.tsx
...
> {
  return {
    primary: {
      // hover:bg-devil-brand-hover focus:bg-devil-brand-hover
    ...
    secondary: {
      ...
      focus: { addRing: true },
      // data-[state=open]:bg-devil-control
 ...
 * - focus: ring-devil-hairline (focus-visible ring)
...
async function createButtonComponent(
  ...
  if (state !== "default" && !disabled && !loading) {
    ...
    if (stateStyle) {
      ...
      // Add focus ring (ring-devil-hairline)
  ...
  if (isCompactShape && !loading) {
    ...
    if (variantStyles.isWhiteText) {
      ...
      bindIconColor(icon, "text-devil-default");
    }
