 ...
 *
 * @see packages/devil-ui/src/components/command-palette/command-palette.tsx
 */
...
const COMPONENT_CLASSES = {
  // Container: "flex max-h-[60vh] flex-col overflow-hidden rounded-lg bg-devil-elevated"
  container: "rounded-lg bg-devil-elevated",
  ...
  // Input header: "flex items-center gap-3 bg-devil-base px-4 py-3"
  inputHeader: "gap-3 bg-devil-base px-4 py-3",
  // Search icon: "h-4 w-4 text-devil-subtle"
  searchIcon: "h-4 w-4 text-devil-subtle",
  // Input: "flex-1 border-none bg-transparent text-base placeholder:text-devil-subtle"
  input: "text-base",
  // List: "z-10 min-h-0 flex-1 overflow-y-auto rounded-b-lg bg-devil-base px-2 py-2 ring-1 ring-devil-line"
  list: "rounded-b-lg bg-devil-base px-2 py-2 ring-1 ring-devil-line",
  // Group: "space-y-0.5"
  group: "space-y-0.5",
  // Group label: "mb-2 px-2 pt-1 text-xs font-semibold text-devil-subtle"
  groupLabel: "px-2 pt-1 text-xs font-semibold text-devil-subtle",
  // Item: "flex w-full items-center gap-3 px-2 py-1.5 text-left cursor-pointer data-[highlighted]:bg-devil-overlay rounded-lg"
  item: "gap-3 px-2 py-1.5 rounded-lg",
  itemHighlighted: "bg-devil-overlay",
  // ResultItem icon container: "flex flex-shrink-0 items-center text-devil-subtle"
  resultIcon: "text-devil-subtle",
  // ResultItem title: "text-base text-devil-default"
  resultTitle: "text-base text-devil-default",
  // ResultItem breadcrumb separator: "h-3 w-3 flex-shrink-0 text-devil-subtle"
  breadcrumbSeparator: "h-3 w-3 text-devil-subtle",
  // ResultItem arrow: "h-4 w-4 flex-shrink-0 text-devil-subtle opacity-0 transition-opacity group-data-[highlighted]:opacity-100"
  resultArrow: "h-4 w-4 text-devil-subtle",
  // External icon: "h-3.5 w-3.5 flex-shrink-0 text-devil-subtle"
  externalIcon: "h-3.5 w-3.5 text-devil-subtle",
  // Empty state: "p-8 text-center text-devil-subtle"
  empty: "p-8 text-devil-subtle",
  // Loading: "flex items-center justify-center p-8"
  loading: "p-8",
  // Footer: "flex items-center justify-between rounded-b-lg bg-devil-elevated px-4 py-3 text-xs text-devil-subtle"
  footer: "rounded-b-lg bg-devil-elevated px-4 py-3 text-xs text-devil-subtle",
  // Highlight mark: "rounded-sm bg-devil-warning/50 text-devil-default"
  highlightMark: "rounded-sm bg-devil-warning/50 text-devil-default",
};
...
const CONFIG = {
  ...
  footerHeight:
    (themeData.tailwind.spacing.scale["3"] || 12) * 2 +
    (themeData.devil.fontSize.xs || 12),
  // py-1.5 = 6px * 2 + content
  itemHeight:
    (themeData.tailwind.spacing.scale["1.5"] || 6) * 2 +
    (themeData.devil.fontSize.base || 14),
  // text-xs + pt-1 + mb-2
  groupLabelHeight:
    (themeData.devil.fontSize.xs || 12) +
 ...
 * Styles from: "flex items-center gap-3 bg-devil-base px-4 py-3"
 */
async function createInputHeader(): Promise<FrameNode> {
  ...
  // Magnifying glass icon (h-4 w-4 text-devil-subtle)
  const searchIcon = getButtonIcon("ph-magnifying-glass", "sm");
  searchIcon.name = "Search Icon";
  bindIconColor(searchIcon, "text-devil-subtle");
...
async function createResultItem(
  ...
  // Item icon (text-devil-subtle)
  const icon = getButtonIcon(iconName, "sm");
  icon.name = "Icon";
  bindIconColor(icon, "text-devil-subtle");
  ...
  if (breadcrumbs && breadcrumbs.length > 0) {
    for (const crumb of breadcrumbs) {
      ...
      bindIconColor(separator, "text-devil-subtle");
      contentWrapper.appendChild(separator);
    }
  }

  // Title text (text-base text-devil-default)
  ...
  if (showArrow && highlighted) {
    ...
    arrow.name = "Arrow";
    bindIconColor(arrow, "text-devil-subtle");
 ...
 * Styles from: "mb-2 px-2 pt-1 text-xs font-semibold text-devil-subtle"
 */
 ...
 * Styles from: "z-10 min-h-0 flex-1 overflow-y-auto rounded-b-lg bg-surface-elevated px-2 py-2 ring-1 ring-devil-line"
 */
async function createResultsList(): Promise<FrameNode> {
  ...
  // Note: The list has ring-1 ring-devil-line and rounded-b-lg in the React component,
 ...
 * Styles from: "rounded border border-devil-line bg-devil-base px-1.5 py-0.5 text-[10px]"
 */
 ...
 * Styles from: "flex items-center justify-between rounded-b-lg bg-devil-elevated px-4 py-3 text-xs text-devil-subtle"
 */
async function createFooter(): Promise<FrameNode> {
  ...
  // bg-devil-elevated from parsed classes
  if (styles.fillVariable) {
    const bgVar = getVariableByName(styles.fillVariable);
    if (bgVar) {
 ...
 * Container styles from: "flex max-h-[60vh] flex-col overflow-hidden rounded-lg bg-devil-elevated"
 */
