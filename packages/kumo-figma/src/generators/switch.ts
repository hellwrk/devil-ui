 ...
 * - Switch track: rounded-full pill, bg-devil-recessed (off) / bg-devil-brand or bg-devil-danger (on)
 * - Switch thumb: circular, bg-white, translates right when checked
 *
 * @see packages/devil-ui/src/components/switch/switch.tsx
 */
...
function createSwitchTrack(
  ...
  // - Unchecked: bg-devil-recessed
  // - Checked + default: bg-devil-brand
  // - Checked + error: bg-devil-danger
...
async function createSwitchGroupComponent(
  ...
  // Error message: text-sm text-devil-danger (14px, but we use xs=12 as closest)
  if (hasError) {
  ...
  // Description: text-sm text-devil-subtle (14px, but we use xs=12 as closest)
  if (hasDescription) {
