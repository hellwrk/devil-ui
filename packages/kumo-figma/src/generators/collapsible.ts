 ...
 *
 * @see packages/devil-ui/src/components/collapsible/collapsible.tsx
 */
 ...
 * "flex cursor-pointer items-center gap-1 text-sm text-devil-link select-none"
 *
 * NOTE: Collapsible has no variants (DEVIL_COLLAPSIBLE_VARIANTS is empty).
 ...
 */
const TRIGGER_BASE_STYLES = "flex items-center gap-1 text-sm text-devil-link";

/**
 * Content panel styles from collapsible.tsx
 * From the inline className in the content div:
 * "my-2 space-y-4 border-l-2 border-devil-fill pl-4"
 *
 * These classes are directly in the JSX, not in a variant function.
 */
const CONTENT_PANEL_STYLES = "my-2 border-l-2 border-devil-fill pl-4";
...
async function createCollapsibleComponent(
  ...
  const iconColorToken =
    state === "disabled" ? "text-devil-inactive" : "text-devil-link";
  ...
  if (open) {
    ...
    // Add left border (border-l-2 border-devil-fill)
