...
/**
 * Helper to verify a value is a valid devil fill variable
 */
function isDevilFillVariable(value: string | null | undefined): boolean {
  if (!value) return false;
  return value.startsWith("color-devil-");
}

/**
 * Helper to verify a value is a valid devil text color variable
 */
function isDevilTextVariable(value: string | null | undefined): boolean {
  if (!value) return false;
  return value.startsWith("text-color-devil-");
}

/**
 * Helper to verify Tailwind classes contain expected patterns
 */
function hasDevilClass(classes: string, pattern: RegExp): boolean {
...
describe("CommandPalette Generator - Component Classes", () => {
  ...
  it("should have container classes with border radius and background", () => {
    ...
    expect(hasDevilClass(classes.container, /rounded-/)).toBe(true);
    expect(hasDevilClass(classes.container, /bg-devil-/)).toBe(true);
  });
  ...
  it("should have inputHeader classes with spacing and background", () => {
    ...
    expect(hasDevilClass(classes.inputHeader, /gap-/)).toBe(true);
    expect(hasDevilClass(classes.inputHeader, /bg-devil-/)).toBe(true);
    expect(hasDevilClass(classes.inputHeader, /p[xy]?-/)).toBe(true);
  });

  it("should have list classes with background and border", () => {
    expect(classes.list).toBeDefined();
    // Verify structural classes exist
    expect(hasDevilClass(classes.list, /bg-devil-/)).toBe(true);
    expect(hasDevilClass(classes.list, /p[xy]?-/)).toBe(true);
    expect(hasDevilClass(classes.list, /ring-/)).toBe(true);
  });

  it("should have item classes with spacing and border radius", () => {
    expect(classes.item).toBeDefined();
    // Verify layout classes exist
    expect(hasDevilClass(classes.item, /gap-/)).toBe(true);
    expect(hasDevilClass(classes.item, /p[xy]?-/)).toBe(true);
    expect(hasDevilClass(classes.item, /rounded-/)).toBe(true);
  });
  ...
  it("should have itemHighlighted classes with background", () => {
    ...
    expect(hasDevilClass(classes.itemHighlighted, /bg-devil-/)).toBe(true);
  });
  ...
  it("should have groupLabel classes with typography and color", () => {
    ...
    expect(hasDevilClass(classes.groupLabel, /text-/)).toBe(true);
    expect(hasDevilClass(classes.groupLabel, /font-/)).toBe(true);
  });

  it("should have footer classes with background, padding, and typography", () => {
    expect(classes.footer).toBeDefined();
    // Verify structural classes exist
    expect(hasDevilClass(classes.footer, /bg-devil-/)).toBe(true);
    expect(hasDevilClass(classes.footer, /p[xy]?-/)).toBe(true);
    expect(hasDevilClass(classes.footer, /text-/)).toBe(true);
  });

  it("should have resultTitle classes with typography", () => {
    expect(classes.resultTitle).toBeDefined();
    // Verify typography classes exist
    expect(hasDevilClass(classes.resultTitle, /text-/)).toBe(true);
  });
});

describe("CommandPalette Generator - Parsed Styles", () => {
  /**
   * Note: Some Devil-specific tokens (bg-surface-2, bg-surface-elevated) are not
  ...
  describe("container styles", () => {
    ...
    it("should parse fillVariable as valid devil color variable", () => {
      // Verify the fill variable follows devil naming convention
      expect(isDevilFillVariable(styles.fillVariable)).toBe(true);
    });
  });
  ...
  describe("list styles", () => {
    ...
    it("should parse strokeVariable as valid devil color variable", () => {
      // Verify the stroke variable follows devil naming convention
      expect(isDevilFillVariable(styles.strokeVariable)).toBe(true);
    });
  ...
  describe("groupLabel styles", () => {
    ...
    it("should parse textVariable as valid devil text color variable", () => {
      // Verify the text variable follows devil naming convention
      expect(isDevilTextVariable(styles.textVariable)).toBe(true);
    });
  });
...
describe("CommandPalette Generator - Base Config", () => {
  ...
  it("should have inputHeader section config with valid devil tokens", () => {
    expect(baseConfig.sections.inputHeader).toBeDefined();
    // Background should be a valid devil color variable
    expect(baseConfig.sections.inputHeader.background).toBe(
      VAR_NAMES.color.elevated,
    );
  });

  it("should have list section config with valid devil tokens", () => {
    expect(baseConfig.sections.list).toBeDefined();
    // Background and border should be valid devil color variables
  ...
  it("should have footer section config with valid devil tokens", () => {
    expect(baseConfig.sections.footer).toBeDefined();
    // Background should be valid devil color, textColor should be valid text color
  ...
  it("should have item config with valid devil tokens", () => {
    expect(baseConfig.item).toBeDefined();
    expect(baseConfig.item.normalBackground).toBeNull();
    // Highlighted background, textColor, iconColor should be valid devil tokens
    ...
    // iconColor uses raw text-devil-* format (not VAR_NAMES)
    expect(baseConfig.item.iconColor).toMatch(/^text-devil-/);
  });
});
...
describe("CommandPalette Generator - Color Token Coverage", () => {
  ...
  it("should use semantic color tokens for item states", () => {
    // Item state colors should reference appropriate devil tokens
    expect(baseConfig.item.highlightedBackground).toBe(VAR_NAMES.color.tint);
    // iconColor uses raw text-devil-* format (not VAR_NAMES)
    expect(baseConfig.item.iconColor).toMatch(/^text-devil-/);
  });
