 ...
 * Container: "min-w-0 rounded-md border border-devil-fill bg-devil-base [&>pre]:p-2.5!"
 */
...
function getPlaceholderText(lang: string): string {
  if (lang === "bash") {
    return "npm install @hellwrk/devil-ui";
  }
  if (lang === "jsonc") {
    return '{ "key": "value" }';
  }
...
async function createCodeBlockComponent(lang: string): Promise<ComponentNode> {
  ...
  // Apply background (bg-devil-base)
  ...
  // Apply text color - text-devil-subtle with fallback to text-devil-default
  const labelVar = getVariableByName(VAR_NAMES.text.label);
  if (labelVar) {
    bindTextColorToVariable(textNode, labelVar.id);
  } else {
    // Fallback to text-devil-default if text-devil-subtle doesn't exist
