 ...
 * - Radio indicator: 16x16 circle, bg-devil-base (off) / bg-devil-contrast (on)
 * - Inner dot: 8x8 circle, bg-devil-base (visible when checked)
 *
 * @see packages/devil-ui/src/components/radio/radio.tsx
 */
...
// Radio component exists in registry but variants are in DEVIL_RADIO_VARIANTS, not props
const _radioComponent = registry.components as Record<string, unknown>;
const _hasRadio = "Radio" in _radioComponent;

// Radio doesn't have a variant prop in props, but has DEVIL_RADIO_VARIANTS in source
// Let's define the variants based on the source code
const RADIO_VARIANTS = {
  values: ["default", "error"],
  classes: {
    default: "ring-devil-line",
...
function createRadioIndicator(
  ...
  if (checked) {
    ...
    // Inner dot is bg-devil-base (white in dark mode, contrasts with bg-devil-contrast)
...
async function createRadioGroupComponent(
  ...
  // Fieldset container: flex flex-col gap-4 rounded-lg border border-devil-line p-4
  ...
  // Legend: text-lg font-medium text-devil-default
  ...
  // Error message: text-sm text-devil-danger
  if (hasError) {
  ...
  // Description: text-sm text-devil-subtle
  if (hasDescription) {
