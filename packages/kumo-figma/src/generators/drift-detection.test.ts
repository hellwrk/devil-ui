 ...
 *
 * This runs automatically in CI via the test-devil job.
 */
...
import registry from "@hellwrk/devil-ui/ai/component-registry.json";
import {
  parseTailwindTheme,
    ...
    if (violations.length > 0) {
      throw new Error(
        `❌ Registry styling integration violations (${violations.length}/${GENERATORS_REQUIRING_STYLING.length}):\n` +
          ...
          `  2. Run: pnpm --filter @hellwrk/devil-ui codegen:registry\n` +
 ...
 * - theme-devil.css (Devil overrides)
...
describe("Figma Plugin - CSS Theme Sync Validation", () => {
  ...
  it("should have generated theme-data.json with correct Devil font sizes", () => {
    // Font sizes are defined in the theme-data.json build process,
    // not extracted from CSS at runtime. Verify the expected values.
    expect(themeData.devil.fontSize.xs).toBe(12);
    expect(themeData.devil.fontSize.sm).toBe(13);
    expect(themeData.devil.fontSize.base).toBe(14);
    expect(themeData.devil.fontSize.lg).toBe(16);
  });

  it("should have generated theme-data.json with correct computed font sizes", () => {
    // Computed values should match Devil overrides
    expect(themeData.computed.fontSize.xs).toBe(themeData.devil.fontSize.xs);
    expect(themeData.computed.fontSize.sm).toBe(themeData.devil.fontSize.sm);
    expect(themeData.computed.fontSize.base).toBe(themeData.devil.fontSize.base);
    expect(themeData.computed.fontSize.lg).toBe(themeData.devil.fontSize.lg);
  });
  ...
  it("should have generated theme-data.json with correct buttonCompactSize", () => {
    ...
    // Path from packages/kumo-figma/src/generators to packages/devil-ui/src/components
    const buttonPath = join(
      __dirname,
      "../../../devil-ui/src/components/button/button.tsx",
    );
    const buttonContent = readFileSync(buttonPath, "utf-8");

    // Extract compactSize mapping from DEVIL_BUTTON_VARIANTS
    ...
    expect(themeData.devil.buttonCompactSize.xs).toBe(extractSizeClass("xs"));
    expect(themeData.devil.buttonCompactSize.sm).toBe(extractSizeClass("sm"));
    expect(themeData.devil.buttonCompactSize.base).toBe(
      extractSizeClass("base"),
    );
    expect(themeData.devil.buttonCompactSize.lg).toBe(extractSizeClass("lg"));

    // Verify expected values
    expect(themeData.devil.buttonCompactSize.xs).toBe(14); // size-3.5 = 14px
    expect(themeData.devil.buttonCompactSize.sm).toBe(26); // size-6.5 = 26px
    expect(themeData.devil.buttonCompactSize.base).toBe(36); // size-9 = 36px
    expect(themeData.devil.buttonCompactSize.lg).toBe(40); // size-10 = 40px
  });
  ...
  it("should use generated values in tailwind-to-figma.ts via import", () => {
    ...
    expect(parserContent).toContain("themeData.devil.fontSize");
...
describe("Figma Plugin - Tailwind v4 theme.css Sync Validation", () => {
  ...
  describe("Tailwind default font sizes (for reference)", () => {
    it("should document Tailwind v4 default font sizes", () => {
      ...
      // Document what Tailwind's defaults are (before Devil overrides)
      ...
      // Devil overrides these in theme-devil.css:
