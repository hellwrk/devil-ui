 ...
 *
 * @see packages/devil-ui/src/components/menubar/menubar.tsx
 * @see packages/devil-ui/src/components/menubar/menubar.stories.tsx (FitContent story)
 */
...
function createMenuOption(iconId: string, isActive: boolean): FrameNode {
  ...
  // Background: bg-devil-fill for inactive, bg-devil-base for active
  if (isActive) {
  ...
  if (icon) {
    icon.name = "Icon";
    // Icon color: fill-devil-contrast (dark on light, light on dark)
    // This maps to color-surface-inverse variable, NOT text-color-surface-inverse
    bindIconColor(icon, "fill-devil-contrast");
