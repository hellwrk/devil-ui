...
describe("Pagination Generator - Color Bindings", () => {
  ...
  it("should use devil semantic tokens for buttons", () => {
    // Button background and border should use devil color tokens
    expect(colors.buttonBackground).toMatch(/^color-devil-/);
    expect(colors.buttonBorder).toMatch(/^color-devil-/);
  });

  it("should use devil semantic tokens for icons", () => {
    // Icon colors should use devil text color tokens
    expect(colors.iconEnabled).toMatch(/^text-color-devil-/);
    expect(colors.iconDisabled).toMatch(/^text-color-devil-/);
    // Enabled and disabled states should be different
    expect(colors.iconEnabled).not.toBe(colors.iconDisabled);
  });

  it("should use devil semantic tokens for input", () => {
    // Input colors should use devil tokens
    expect(colors.inputBackground).toMatch(/^color-devil-/);
    expect(colors.inputBorder).toMatch(/^color-devil-/);
    expect(colors.inputText).toMatch(/^text-color-devil-/);
  });

  it("should use devil semantic token for showing text", () => {
    expect(colors.showingTextLabel).toMatch(/^text-color-devil-/);
  });
});
