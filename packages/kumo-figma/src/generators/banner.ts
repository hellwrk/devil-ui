...
/**
 * Banner base styles from DEVIL_BANNER_BASE_STYLES in banner.tsx
    ...
    variants: config.values.map((variant) => {
      ...
      return {
        ...
        icon: {
          ...
          iconSize: FONT_SIZE.lg, // 16px (Devil's text-lg, slightly larger than text-base)
        },
        // Text properties
        text: {
...
async function createBannerComponent(variant: string): Promise<ComponentNode> {
  ...
  // Apply fill from variant (bg-devil-info/20, bg-devil-warning/20, bg-devil-danger/20)
  ...
  const iconSize = FONT_SIZE.lg; // 16px (Devil's text-lg, slightly larger than text-base)
