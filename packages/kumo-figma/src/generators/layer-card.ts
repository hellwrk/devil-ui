 ...
 *
 * @see packages/devil-ui/src/components/layer-card/layer-card.tsx
 */
...
const FALLBACK_LAYER_CARD_CONFIG = {
  ...
  secondary: {
    ...
    fontSize: FONT_SIZE.lg, // 16px from theme-devil.css
 ...
 *
 * Structure:
 * - Root frame (bg-surface-2, ring-devil-line)
 * - Secondary frame (header with text-devil-strong)
 *   - Primary frame (bg-surface, ring-devil-fill)
 *
...
async function createLayerCardComponent(): Promise<ComponentNode> {
  ...
  // Apply root background (bg-devil-elevated)
  const rootBgVar = getVariableByName(VAR_NAMES.color.elevated);
  if (rootBgVar) {
    bindFillToVariable(component, rootBgVar.id);
  } else {
    console.log(
      "LayerCard: color-devil-elevated variable not found, using fallback",
    );
  ...
  // Apply root ring (ring-devil-line)
  ...
  if (arrowIcon) {
    arrowIcon.name = "Icon";
    // Bind icon color to text-devil-strong (same as the title text)
    bindIconColor(arrowIcon, "text-devil-strong");
  ...
  // Apply primary background (bg-devil-base)
  ...
  // Apply primary ring (ring-devil-fill)
...
export function getLayerCardContentConfig() {
  return {
    ...
    primary: {
      content: "Get started with Devil",
    },
  };
}
