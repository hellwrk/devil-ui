 ...
 *
 * @see packages/devil-ui/src/components/dialog/dialog.tsx
 */
...
async function createButton(
  ...
  // Create button label
  // For sm buttons, use Tailwind's text-sm (14px) rather than Devil's 13px override
  const fontSize =
  ...
  if (isPrimary) {
    ...
    // text-devil-default for secondary
...
async function createDialogComponent(size: string): Promise<ComponentNode> {
  ...
  // Apply title text color (text-devil-default)
  ...
  // Color is text-devil-subtle to match code: className="text-devil-subtle hover:text-devil-default"
  const closeIconName = "ph-x";
  const closeIcon = getButtonIcon(closeIconName, "base");
  closeIcon.name = "Close";

  // Apply icon color (text-devil-subtle to match code)
  bindIconColor(closeIcon, "text-devil-subtle");
