...
describe("Code Generator - Base Styles Parsing", () => {
   ...
   * Base styles: "m-0 w-auto rounded-none border-none bg-transparent p-0 font-mono text-sm leading-[20px] text-devil-subtle"
   */
...
describe("Code Generator - Color Token Coverage", () => {
  ...
  it("should map text-devil-strong color token", () => {
    const parsed = parseTailwindClasses("text-devil-strong");
    expect(parsed.textVariable).toBeDefined();
    expect(typeof parsed.textVariable).toBe("string");
  });
  ...
  it("should map bg-devil-base for CodeBlock container", () => {
    const parsed = parseTailwindClasses("bg-devil-base");
    expect(parsed.fillVariable).toBeDefined();
    expect(typeof parsed.fillVariable).toBe("string");
  });

  it("should map border-devil-fill for CodeBlock container", () => {
    const parsed = parseTailwindClasses("border border-devil-fill");
    expect(parsed.hasBorder).toBe(true);
