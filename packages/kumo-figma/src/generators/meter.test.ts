...
describe("Meter Generator - Registry Validation", () => {
  ...
  it("should have expected colors in registry", () => {
    ...
    // All colors should be devil semantic tokens
...
describe("Meter Generator - Color Bindings", () => {
  ...
  it("should use semantic color tokens", () => {
    ...
    // Label should use a devil text color token
    expect(bindings.label).toMatch(/^text-color-devil-/);

    // Value should use a devil text color token
    expect(bindings.value).toMatch(/^text-color-devil-/);

    // Track should use a devil color token
    expect(bindings.track).toMatch(/^color-devil-/);

    // Indicator should use a devil color token
    expect(bindings.indicator).toMatch(/^color-devil-/);
  });
