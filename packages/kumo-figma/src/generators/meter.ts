 ...
 *
 * Meter has no variants (DEVIL_METER_VARIANTS is empty). The React component uses:
 * - Root: "flex w-full flex-col gap-2"
 * - Label: "text-xs text-devil-strong" (12px, text-devil-strong token)
 * - Value: "text-sm font-medium text-devil-default tabular-nums" (14px, 500 weight, text-devil-default token)
 * - Track: "h-2 w-full rounded-full bg-devil-fill" (height 8px, bg-devil-fill token)
 * - Indicator: "rounded-full bg-linear-to-r from-devil-brand" (primary gradient)
 */
 ...
 * - Label: text-devil-strong (text-xs text-devil-strong)
 * - Value: text-devil-default (text-sm font-medium text-devil-default)
 * - Track: bg-devil-fill (from Track className)
 * - Indicator: bg-devil-brand (from bg-linear-to-r from-devil-brand)
 */
export function getMeterColorBindings() {
  return {
    label: VAR_NAMES.text.label,
    value: VAR_NAMES.text.surface,
    track: VAR_NAMES.color.fill, // Note: React uses bg-devil-fill
...
export function getMeterTypographyConfig() {
  return {
    label: {
      fontSize: FONT_SIZE.xs, // 12px from theme-devil.css (text-xs)
      fontWeight: FALLBACK_VALUES.fontWeight.normal, // 400
      colorToken: VAR_NAMES.text.label,
      source: "text-xs text-devil-strong",
    },
    value: {
      fontSize: FONT_SIZE.sm, // 13px from theme-devil.css (text-sm, NOT 14px!)
      fontWeight: FALLBACK_VALUES.fontWeight.medium, // 500 (font-medium)
      colorToken: VAR_NAMES.text.surface,
      source: "text-sm font-medium text-devil-default tabular-nums",
    },
  };
}
...
async function createMeterComponent(
  ...
  // Bind track background to bg-devil-fill variable
