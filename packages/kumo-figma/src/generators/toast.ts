 ...
 * - Description text (text-devil-subtle)
 * - Close button with X icon (text-devil-subtle)
 *
 * Reads styles from component-registry.json (the source of truth).
 * Uses real icons from the Icon Library page.
 *
 * @see packages/devil-ui/src/components/toast/toast.tsx
 */
...
async function createToastComponent(): Promise<ComponentNode> {
  ...
  // Apply border (border-devil-fill)
  ...
  // Apply icon color (text-devil-subtle)
 ...
 * toast.tsx (DEVIL_TOAST_STYLING) → component-registry.json → toast.ts
 */
