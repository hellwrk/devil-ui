 ...
 * - Tooltip has bg-devil-base, text-devil-default, rounded-md, px-2.5 py-1.5 padding
 * - Includes an arrow/pointer
 *
 * The Tooltip has:
 * - Container with bg-devil-base, text-devil-default, rounded-md, px-2.5 py-1.5
 ...
 * - Arrow uses fill matching bg-devil-base
 *
 * @see packages/devil-ui/src/components/tooltip/tooltip.tsx
 */
 ...
 * "rounded-md bg-devil-base px-2.5 py-1.5 text-sm text-devil-default"
 * "shadow-lg outline outline-devil-fill"
 */
 ...
 * 1. ArrowFill - The main arrow body (fill-devil-base)
 * 2. ArrowOuterStroke - Border visible in light mode (fill-devil-arrow-edge)
 * 3. ArrowInnerStroke - Border stroke (fill-devil-arrow-stroke) - matches tooltip outline
 *
...
function createTooltipArrow(): FrameNode {
  ...
  if (children.length >= 3) {
    // First path: Arrow Fill (fill-devil-base)
    ...
    // Second path: Arrow Edge (fill-devil-arrow-edge)
    ...
    // Third path: Arrow Stroke (fill-devil-arrow-stroke)
...
async function createTooltipComponent(): Promise<ComponentNode> {
  ...
  // Apply background fill (bg-devil-base)
  const bgVar = getVariableByName(VAR_NAMES.color.base);
  if (bgVar) {
    bindFillToVariable(tooltipBox, bgVar.id);
  }

  // Apply outline stroke (outline outline-devil-fill)
  ...
  // Apply text color (text-devil-default)
...
export function getTooltipStylingConfig() {
  return {
    popup: {
      ...
      text: "text-devil-default",
      paddingX: "px-2.5", // 10px
      paddingY: "py-1.5", // 6px
      fontSize: "text-sm", // 14px
      borderRadius: "rounded-md",
      outline: "outline-devil-fill", // Provides visual separation in dark mode
    },
    arrow: {
      // Arrow matches React component's ArrowSvg with 3 paths:
      // - fill-devil-base: main arrow body
      // - fill-devil-arrow-edge: outer stroke (light mode)
      // - fill-devil-arrow-stroke: border stroke (matches outline-devil-fill)
      fill: "fill-devil-base",
      arrowEdge: "fill-devil-arrow-edge",
      arrowStroke: "fill-devil-arrow-stroke",
      width: ARROW_WIDTH,
      height: ARROW_HEIGHT,
    },
  };
