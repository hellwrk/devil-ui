...
│   │   ├── opacity-extractor.ts   # `bg-devil-brand/70` → opacity data
...
- **Opacity variables created at runtime**: `getOrCreateVariableWithOpacity("color-devil-info/20")` alpha-blends against white/black on the fly.
...
- **26 `@ts-ignore` instances**: All for `figma.combineAsVariants()` type mismatch at runtime
