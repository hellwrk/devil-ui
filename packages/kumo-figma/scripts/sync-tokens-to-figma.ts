 ...
 *   FIGMA_FILE_KEY (optional) - Target Figma file, defaults to devil file
 */
...
import {
  THEME_CONFIG,
  AVAILABLE_THEMES,
} from "../../devil-ui/scripts/theme-generator/config.js";
import type { TokenDefinition } from "../../devil-ui/scripts/theme-generator/types.js";
...
const OPACITY_MODIFIERS: Record<string, number[]> = {
  // Banner variants: bg-info/20, bg-alert/20, bg-devil-danger/20
  info: [20],
  alert: [20],
  error: [20, 70, 90],
  // Button variants: bg-primary/50, bg-primary/70, bg-devil-control/50
  primary: [50, 70],
  secondary: [50],
};
...
function getColorTokensFromConfig(): {
  ...
  // Initialize override maps for non-devil themes
  ...
  for (const [tokenName, def] of Object.entries(THEME_CONFIG.text)) {
    ...
    if (typedDef.theme.devil) {
      baseTokens.push({
        ...
        light: resolveColor(typedDef.theme.devil.light),
        dark: resolveColor(typedDef.theme.devil.dark),
      });
    }
...
async function runSyncCommand(): Promise<void> {
  ...
  const typographyCollectionName = "devil-typography";
