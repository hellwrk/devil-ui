...
import { Badge, Input, LayerCard, Tabs } from "@hellwrk/devil-ui";
    ...
    <LayerCard className="w-full max-w-[540px]">
      ...
      <LayerCard.Primary>
        ...
        <div className="-mx-1 text-sm">
          {/* Header */}
          <div className="grid grid-cols-[1fr_auto_auto] gap-x-4 border-b border-devil-fill px-1 pb-2 text-xs font-medium text-devil-subtle">
            <span>Origin</span>
          ...
          {filtered.map((row, i) => (
            <div
              key={row.origin}
              className={`grid grid-cols-[1fr_auto_auto] items-center gap-x-4 px-1 py-2.5 ${i < filtered.length - 1 ? "border-b border-devil-hairline" : ""}`}
            >
              <span className="truncate font-medium text-devil-default">
                {row.origin}
              </span>
              ...
              <span className="w-20 text-right text-devil-subtle">
                {row.duration}
              </span>
            </div>
          ))}
        ...
        {/* Footer */}
        <div className="-mx-1 border-t border-devil-fill pt-2 text-xs text-devil-subtle">
