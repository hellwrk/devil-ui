---
layout: ~/layouts/MdxDocLayout.astro
title: "Colors"
description: "devil-ui uses semantic color tokens that automatically adapt to light and dark mode. Use these classes in your Tailwind CSS instead of raw color values."
---

import {
  TailwindColorTokens,
  StatusBannerDemo,
} from "~/components/demos/ColorsDemo";
import ComponentExample from "~/components/docs/ComponentExample.astro";
import { Badge, Banner } from "@hellwrk/devil-ui";
import { InfoIcon } from "@phosphor-icons/react";

## Usage

Always use semantic tokens instead of raw Tailwind colors. This ensures your UI automatically adapts to light and dark mode, and that your components remain consistent across themes.

#### Correct

```tsx
<div className="bg-devil-base text-devil-default border-devil-hairline">
  <button className="bg-devil-brand text-white">Primary</button>
  <button className="bg-devil-control text-devil-default">Secondary</button>
</div>
```

#### Incorrect

```tsx
{
  /* Never use raw Tailwind colors */
}
<div className="bg-white dark:bg-gray-900 text-black dark:text-white">
  <button className="bg-blue-500">Primary</button>
</div>;
```

> Lint rules enforce this: The `no-primitive-colors` rule will flag any raw Tailwind colors like `bg-blue-500`.

## Mode

Set `data-mode` on a parent element to control light/dark mode. Never use Tailwind's `dark:` variant — semantic tokens handle dark mode automatically via CSS `light-dark()`.

```tsx
// Set mode on html or body
<html data-mode="light">  // Light mode
<html data-mode="dark">   // Dark mode

// Components automatically adapt - no dark: variants needed
<div className="bg-devil-base text-devil-default" />
```

## Themes

Themes override semantic token values while preserving the same token names. Set `data-theme` on a parent element to apply a theme.

### Available Themes

- `devil` — Default theme (no attribute needed)
- `fedramp` — Government compliance styling

```tsx
// Apply a theme to a section or the whole app
<div data-theme="fedramp">
  {/* All Devil components inside use fedramp token overrides */}
  <Button>FedRAMP Styled</Button>
</div>

// Themes work with both light and dark mode
<html data-mode="dark" data-theme="fedramp">
```

### Theme Generator

Themes are defined in a centralized config and generated as CSS files. The theme generator ensures consistency across all themes.

```bash

# List all tokens and their theme overrides

pnpm --filter @hellwrk/devil-ui codegen:themes --list

# Generate theme CSS files

pnpm --filter @hellwrk/devil-ui codegen:themes

# Preview changes without writing files

pnpm --filter @hellwrk/devil-ui codegen:themes --dry-run
```

Theme config: `packages/devil-ui/scripts/theme-generator/config.ts`

### Creating a New Theme

Add theme overrides in the config file. Only override tokens that need to change — all other tokens inherit from the base devil theme.

```ts
// In scripts/theme-generator/config.ts
export const THEME_CONFIG: ThemeConfig = {
  color: {
    "devil-base": {
      newName: "",
      theme: {
        devil: {
          light: "var(--color-white, #fff)",
          dark: "var(--color-black, #000)",
        },
        // Add your theme override
        myTheme: {
          light: "#f0f4f8",
          dark: "#1a1f2e",
        },
      },
    },
    // ... other tokens
  },
};

// Add to available themes
export const AVAILABLE_THEMES = ["devil", "fedramp", "myTheme"] as const;
```

Then run `pnpm codegen:themes` to generate the CSS.

## Semantic Tokens

We use semantic tokens to group colors by purpose. Use the token that matches the role of the element, not the color you want to achieve.

Semantic tokens are named by **role**, not by hue. A token like `bg-devil-danger` communicates intent — it doesn't imply a specific shade of red, and its exact value can change per theme or color mode without touching your component code.

### Surface Hierarchy

Surfaces establish depth and layering in the UI. Use them in order from the outermost background inward.

<div style="overflow-x: auto;">
  <table>
    <colgroup>
      <col style="width: 22ch;" />
      <col />
    </colgroup>
    <thead>
      <tr>
        <th>Token</th>
        <th>Purpose</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td><code>bg-devil-canvas</code></td>
        <td>The outermost page background — sits behind everything</td>
      </tr>
      <tr>
        <td><code>bg-devil-base</code></td>
        <td>Default component background</td>
      </tr>
      <tr>
        <td><code>bg-devil-elevated</code></td>
        <td>Slightly elevated surface, e.g. <code>LayerCard.Secondary</code></td>
      </tr>
      <tr>
        <td><code>bg-devil-recessed</code></td>
        <td>Recessed surface with a subtly darker fill, e.g. segmented <code>Tabs</code> background</td>
      </tr>
      <tr>
        <td><code>bg-devil-tint</code></td>
        <td>Subtle tinted background for tables or hover states</td>
      </tr>
      <tr>
        <td><code>bg-devil-contrast</code></td>
        <td>High-contrast, inverted background</td>
      </tr>
    </tbody>
  </table>
</div>

### Brand

<div style="overflow-x: auto;">
  <table>
    <colgroup>
      <col style="width: 22ch;" />
      <col />
    </colgroup>
    <thead>
      <tr>
        <th>Token</th>
        <th>Purpose</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td><code>bg-devil-brand</code></td>
        <td>Primary brand background</td>
      </tr>
      <tr>
        <td><code>bg-devil-brand-hover</code></td>
        <td>Hover state for brand backgrounds</td>
      </tr>
    </tbody>
  </table>
</div>

### Semantic Status Colors

Each status color comes in two variants: a solid color for icons and indicators, and a `-tint` variant for background fills behind content (i.e. `Badge` or `Banner`).

<div style="overflow-x: auto;">
  <table>
    <colgroup>
      <col style="width: 22ch;" />
      <col />
    </colgroup>
    <thead>
      <tr>
        <th>Token</th>
        <th>Purpose</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td><code>bg-devil-info</code></td>
        <td>Info indicator</td>
      </tr>
      <tr>
        <td><code>bg-devil-success</code></td>
        <td>Success indicator</td>
      </tr>
      <tr>
        <td><code>bg-devil-warning</code></td>
        <td>Warning indicator</td>
      </tr>
      <tr>
        <td><code>bg-devil-danger</code></td>
        <td>Error/destructive indicator</td>
      </tr>
    </tbody>
  </table>
</div>

Use the solid token `bg-devil-*` for status dots, `fill-devil-*` for icons, and `border-devil-*`, `ring-devil-*` for borders and rings. Banners and badges use the `-tint` variant with varying opacity values.

<ComponentExample demo="StatusBannerDemo">
  <StatusBannerDemo client:visible />
</ComponentExample>

### Text Colors

<div style="overflow-x: auto;">
  <table>
    <colgroup>
      <col style="width: 22ch;" />
      <col />
    </colgroup>
    <thead>
      <tr>
        <th>Token</th>
        <th>Purpose</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td><code>text-devil-default</code></td>
        <td>Primary body text</td>
      </tr>
      <tr>
        <td><code>text-devil-strong</code></td>
        <td>Stronger text contrast than default for headers and important labels</td>
      </tr>
      <tr>
        <td><code>text-devil-subtle</code></td>
        <td>Muted text for descriptions, captions, or secondary labels</td>
      </tr>
      <tr>
        <td><code>text-devil-inactive</code></td>
        <td>Disabled or inactive text</td>
      </tr>
      <tr>
        <td><code>text-devil-placeholder</code></td>
        <td>Placeholder text in inputs</td>
      </tr>
      <tr>
        <td><code>text-devil-inverse</code></td>
        <td>Text intended for use on high-contrast or inverted backgrounds</td>
      </tr>
      <tr>
        <td><code>text-devil-link</code></td>
        <td>Link text</td>
      </tr>
      <tr>
        <td><code>text-devil-info</code></td>
        <td>Info-colored text</td>
      </tr>
      <tr>
        <td><code>text-devil-success</code></td>
        <td>Success-colored text</td>
      </tr>
      <tr>
        <td><code>text-devil-warning</code></td>
        <td>Warning-colored text</td>
      </tr>
      <tr>
        <td><code>text-devil-danger</code></td>
        <td>Error/destructive text</td>
      </tr>
    </tbody>
  </table>
</div>

> Semantic text colors (i.e. <code>text-devil-success</code>) are darker by default to provide better contrast and readability against <code>tint-*</code> backgrounds.

### Borders & Rings

<div style="overflow-x: auto;">
  <table>
    <colgroup>
      <col style="width: 22ch;" />
      <col />
    </colgroup>
    <thead>
      <tr>
        <th>Token</th>
        <th>Purpose</th>
      </tr>
    </thead>
    <tbody>
      <tr>
        <td>
          <span className="inline-flex items-center gap-2">
            <code>devil-hairline</code>
          </span>
        </td>
        <td>A border/ring color to distinguish between flat surfaces where no shadow is present (i.e. <code>LayerCard</code>).</td>
      </tr>
      <tr>
        <td><code>devil-line</code></td>
        <td>A thicker border/ring color that defines the edge of an elevated surface alongside a shadow.</td>
      </tr>
    </tbody>
  </table>
</div>

## Token Reference

Toggle the theme in the header to see how tokens adapt. Tokens marked as "global" are explicit opt-in classes available regardless of theme.

<TailwindColorTokens client:load />
