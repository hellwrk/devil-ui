...
```tsx
import { Collapsible } from "@hellwrk/devil-ui";
```

### Granular

```tsx
import { Collapsible } from "@hellwrk/devil-ui/components/collapsible";
```
...
Use `DefaultTrigger` and `DefaultPanel` for the classic devil-ui style:
...
<Collapsible.Root open={open} onOpenChange={setOpen}>
  ...
  <Collapsible.Panel className="mt-2 p-4 bg-devil-tint rounded-lg">
