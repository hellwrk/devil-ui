...
description: "Generate Figma components from devil-ui definitions using the Figma plugin."
---
...
The Kumo Figma plugin generates production-quality Figma components directly from Devil component definitions. This keeps design and code in sync — components in Figma are generated from the same source of truth as the React components you use in code.
...
3. Navigate to `packages/kumo-figma/src/manifest.json` in your local devil repository

### 3. Run the Plugin

Once imported, the plugin appears in your development plugins list.

1. Open any Figma file (or the Devil design file if you have access)
...
Before generating components, sync Devil's semantic color tokens to Figma. This creates the `kumo-colors` variable collection that components bind to for consistent theming.
...
This parses tokens from `theme-devil.css`, resolves `light-dark()` values for both modes, and pushes them to Figma via the Variables API.
...
The plugin generates a **"ui kit"** page containing ComponentSets for each Devil component:
...
# 1. Make changes to Devil components

# ...edit packages/devil-ui/src/components/...

# 2. Regenerate component registry

pnpm --filter @hellwrk/devil-ui codegen:registry
...
When you add a new component to Devil, create a corresponding Figma generator:
