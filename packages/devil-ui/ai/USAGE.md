...
```css
@source "path/to/node_modules/devil-ui/dist/**/*.{js,jsx,ts,tsx}";
@import "@hellwrk/devil-ui/styles";
@import "tailwindcss";
```

- **`@source`** — Tells Tailwind to scan devil's compiled JS for class names. Without this, devil component internals will be unstyled. Adjust the relative path so it resolves to your project's `node_modules/`.
- **Import order matters** — `devil-ui/styles` must come **before** `@import "tailwindcss"` so devil's `@theme` tokens are registered first.
...
1. **Semantic tokens only** — use `bg-devil-base`, `text-devil-default`, etc. Never use raw Tailwind colors (`bg-blue-500`).
2. **No `dark:` variant** — light/dark mode is handled automatically via CSS `light-dark()`. Never add `dark:` prefixes.
3. **Merge classes with `cn()`** — import from `devil-ui` and use it to combine class names.
...
| Token              | Purpose                                                                      |
| ------------------ | ---------------------------------------------------------------------------- |
| `bg-devil-canvas`   | The outermost page background — sits behind everything                       |
| `bg-devil-base`     | Default component background                                                 |
| `bg-devil-elevated` | Slightly elevated surface, e.g. `LayerCard.Secondary`                        |
| `bg-devil-recessed` | Recessed surface with a subtly darker fill, e.g. segmented `Tabs` background |
| `bg-devil-tint`     | Subtle tinted background for tables or hover states                          |
| `bg-devil-contrast` | High-contrast, inverted background                                           |

### Brand

| Token                 | Purpose                           |
| --------------------- | --------------------------------- |
| `bg-devil-brand`       | Primary brand background          |
| `bg-devil-brand-hover` | Hover state for brand backgrounds |

### Semantic Status Colors

| Token             | Purpose                         |
| ----------------- | ------------------------------- |
| `bg-devil-info`    | Info indicator (icon, dot, bar) |
| `bg-devil-success` | Success indicator               |
| `bg-devil-warning` | Warning indicator               |
| `bg-devil-danger`  | Error/destructive indicator     |
...
| Token                   | Purpose                                                        |
| ----------------------- | -------------------------------------------------------------- |
| `text-devil-default`     | Primary body text                                              |
| `text-devil-strong`      | Secondary text with slightly less contrast than default        |
| `text-devil-subtle`      | Muted text for descriptions, captions, or secondary labels     |
| `text-devil-inactive`    | Disabled or inactive text                                      |
| `text-devil-placeholder` | Placeholder text in inputs                                     |
| `text-devil-inverse`     | Text intended for use on high-contrast or inverted backgrounds |
| `text-devil-link`        | Link text                                                      |
| `text-devil-info`        | Info-colored text                                              |
| `text-devil-success`     | Success-colored text                                           |
| `text-devil-warning`     | Warning-colored text                                           |
| `text-devil-danger`      | Error/destructive text                                         |

### Borders & Rings

| Token           | Purpose                                                                                                 |
| --------------- | ------------------------------------------------------------------------------------------------------- |
| `devil-hairline` | A border/ring color to distinguish between flat surfaces where no shadow is present (i.e. `LayerCard`). |
| `devil-line`     | A thicker border/ring color that defines the edge of an elevated surface alongside a shadow.            |

## Icons

Devil uses **[Phosphor Icons](https://phosphoricons.com/)** via `@phosphor-icons/react`:
...
```
devil-ui/ai/component-registry.json
```
