...
// when consumers import from devil-ui, causing build failures.
...
try {
  const standaloneInput = join(srcDir, "devil-standalone.css");
  const standaloneOutput = join(distDir, "devil-standalone.css");
  ...
  console.log("✓ Compiled devil-standalone.css");
} catch (error) {
...
// Copy block source files to dist for CLI `devil add` command
