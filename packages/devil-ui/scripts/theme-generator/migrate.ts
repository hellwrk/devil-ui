#!/usr/bin/env node
/**
 * Devil Token Migration Tool
 *
 * Complete migration workflow:
 * 1. Renames Tailwind classes in source files (bg-surface -> bg-devil-base)
 * 2. Updates config.ts: newName becomes key, newName field becomes ""
 * 3. Regenerates theme CSS with new names
 *
 * Usage:
 *   pnpm migrate:tokens              # Migrate devil codebase
...
function rewriteConfig(renameMap: TokenRenameMap): string {
  ...
  lines.push(`export const AVAILABLE_THEMES = ["devil", "fedramp"] as const;`);
...
function parseArgs(
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (arg === "--path" && args[i + 1]) {
      ...
      updateConfig = false; // External projects don't update devil's config
    } else if (arg === "--dry-run") {
      dryRun = true;
...
1. Updates class names in source files (bg-surface -> bg-devil-base)
...
EXAMPLE - Renaming "surface" to "devil-base":

  Before config.ts:
    surface: { newName: "devil-base", theme: { ... } }

  After migration:
    "devil-base": { newName: "", theme: { ... } }

  Class changes:
    bg-surface -> bg-devil-base
    text-surface -> text-devil-base
    hover:bg-surface/50 -> hover:bg-devil-base/50
`);
}
...
function printExportMap(): void {
  ...
  console.log(
    JSON.stringify(
      {
        meta: {
          description: "Devil token migration map",
...
async function main(): Promise<void> {
  ...
  if (!hasPendingMigrations(renameMap)) {
    ...
    console.log('  surface: { newName: "devil-base", theme: { ... } }');
    return;
  }

  console.log("Devil Token Migration\n");
