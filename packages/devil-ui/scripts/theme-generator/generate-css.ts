/**
 * CSS Generator for Devil Themes
 *
...
function pushModeScopedBaseVariables(
  ...
  const lightSelector =
    themeName === "devil" ? `:root, ${themeSelector}` : `${themeSelector}`;
  const darkSelector =
    themeName === "devil"
...
/**
 * Generate the base devil theme CSS
 * This contains all tokens with their devil theme values
 */
export function generateDevilThemeCSS(
  config: ThemeConfig,
  useNewNames = false,
): string {
  ...
  for (const [tokenName, def] of Object.entries(config.text)) {
    const name = useNewNames ? def.newName : tokenName;
    const devilColors = def.theme.devil;

    lines.push(
      `  --text-color-${name}: ${lightDark(devilColors.light, devilColors.dark)};`,
    );
    lines.push("");
  }
  ...
  for (const [tokenName, def] of Object.entries(config.color)) {
    ...
    lines.push(
      `  --color-${name}: ${lightDark(devilColors.light, devilColors.dark)};`,
    );
    lines.push("");
  }
  ...
  pushModeScopedBaseVariables(lines, config, "devil", useNewNames, {
 ...
 * Only includes tokens that differ from the base devil theme
 */
export function generateThemeOverrideCSS(
  ...
  if (themeName === "devil") {
    throw new Error("Use generateDevilThemeCSS for the base devil theme");
  }
...
export function generateAllThemes(
  ...
  // Always generate base devil theme
  if (themes.includes("devil")) {
    files.set(
      "theme-devil.css",
      generateDevilThemeCSS(THEME_CONFIG, useNewNames),
    );
  }
