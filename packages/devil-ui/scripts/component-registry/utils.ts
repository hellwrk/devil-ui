...
/**
 * Parse theme-devil.css to extract semantic color names from --color-* and --text-color-* variables.
...
export function parseSemanticColorNames(): string[] {
  ...
  const themePath = join(__dirname, "../../src/styles/theme-devil.css");
 ...
 * Returns array of Tailwind classes like ["bg-devil-base", "text-devil-subtle"].
 */
export function extractSemanticColors(sourceFile: string): string[] {
  try {
    ...
    // Match patterns like: text-surface, bg-primary, border-color, ring-devil-line
    // Also handles variants like: hover:bg-primary, disabled:text-devil-subtle
