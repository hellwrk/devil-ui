 ...
 *
 * Extracts DEVIL_*_VARIANTS, DEVIL_*_DEFAULT_VARIANTS, and DEVIL_*_STYLING from component files
...
/**
 * Extract DEVIL_*_BASE_STYLES from a component file.
...
export function extractBaseStylesFromFile(filePath: string): string | null {
  try {
    ...
    // Match: export const DEVIL_*_BASE_STYLES = "..." or '...' or `...`
    // Handles multi-line strings with template literals
    const baseStylesMatch = content.match(
      /export\s+const\s+DEVIL_\w+_BASE_STYLES\s*=\s*["'`]([^"'`]+)["'`]/,
    );
...
/**
 * Extract DEVIL_*_VARIANTS and DEVIL_*_DEFAULT_VARIANTS from a component file.
...
export function extractVariantsFromFile(
  ...
  try {
    ...
    // Find DEVIL_*_VARIANTS export start position
    const variantsStartMatch = content.match(
      /export\s+const\s+DEVIL_\w+_VARIANTS\s*=\s*/,
    );
    // Find DEVIL_*_DEFAULT_VARIANTS export start position
    const defaultsStartMatch = content.match(
      /export\s+const\s+DEVIL_\w+_DEFAULT_VARIANTS\s*=\s*/,
    );
...
export function extractStylingFromFile(
  ...
  try {
    ...
    // Find DEVIL_*_STYLING export start position
    const stylingStartMatch = content.match(
      /export\s+const\s+DEVIL_\w+_STYLING\s*=\s*/,
    );
