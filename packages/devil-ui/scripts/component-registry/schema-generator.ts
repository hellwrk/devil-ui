/**
 * Zod Schema Generator for Devil Component Registry
 *
...
export function generateSchemasFile(registry: ComponentRegistry): string {
  ...
  lines.push(`export type DevilComponentType = ${typeUnion};`);
  lines.push("");
  lines.push("export const DevilComponentTypeSchema = z.enum([");
  for (const name of componentNames) {
    lines.push(`  "${name}",`);
  }
  lines.push("]);");
  ...
  lines.push("  type: DevilComponentTypeSchema,");
  ...
  lines.push(" */");
  lines.push(
    `export const DEVIL_COMPONENT_NAMES = [${componentNames.map((n) => `"${n}"`).join(", ")}] as const;`,
  );
  lines.push("");
