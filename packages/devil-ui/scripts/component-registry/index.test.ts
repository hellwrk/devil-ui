...
describe("extractStateClasses", () => {
  ...
  it("extracts hover state", () => {
    const classes = "bg-devil-brand hover:bg-primary-dark";
  ...
  it("extracts focus state", () => {
    const classes = "ring-devil-line focus:ring-devil-hairline";
    expect(extractStateClasses(classes)).toEqual({
      focus: "focus:ring-devil-hairline",
    });
  });

  it("extracts multiple states", () => {
    const classes =
      "bg-devil-base hover:bg-devil-elevated focus:ring-devil-hairline disabled:opacity-50";
    expect(extractStateClasses(classes)).toEqual({
      hover: "hover:bg-devil-elevated",
      focus: "focus:ring-devil-hairline",
  ...
  it("returns empty object for no state classes", () => {
    const classes = "bg-devil-base text-devil-default rounded-lg";
    expect(extractStateClasses(classes)).toEqual({});
  });

  it("combines multiple classes of same state", () => {
    const classes = "hover:bg-devil-brand hover:text-white";
    expect(extractStateClasses(classes)).toEqual({
      hover: "hover:bg-devil-brand hover:text-white",
    });
  });
});
...
describe("detectComponentExportsFromIndex", () => {
  it("detects multiple components with matching props from a barrel", () => {
    const dir = mkdtempSync(join(tmpdir(), "devil-barrel-test-"));
    try {
      writeFileSync(
        join(dir, "index.ts"),
        `
...
function writeTempFile(content: string): {
  ...
  const dir = mkdtempSync(join(tmpdir(), "devil-test-"));
