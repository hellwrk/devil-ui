...
describe("enforce-variant-standard", () => {
  describe("Valid component exports", () => {
    it("should pass with DEVIL_BUTTON_VARIANTS + DEVIL_BUTTON_DEFAULT_VARIANTS", () => {
      const reports = runRule("src/components/button/button.tsx", [
        "DEVIL_BUTTON_VARIANTS",
        "DEVIL_BUTTON_DEFAULT_VARIANTS",
      ]);

      expect(reports).toHaveLength(0);
    });

    it("should pass with DEVIL_BADGE_VARIANTS + DEVIL_BADGE_DEFAULT_VARIANTS + DEVIL_BADGE_BASE_STYLES", () => {
      const reports = runRule("src/components/badge/badge.tsx", [
        "DEVIL_BADGE_VARIANTS",
        "DEVIL_BADGE_DEFAULT_VARIANTS",
    ...
    it("should pass with kebab-case component names (toast-provider)", () => {
      const reports = runRule(
        ...
        [
          "DEVIL_TOAST_PROVIDER_VARIANTS",
          "DEVIL_TOAST_PROVIDER_DEFAULT_VARIANTS",
        ],
      );
  ...
  describe("Invalid exports - Missing DEVIL_ prefix", () => {
    it("should fail when VARIANTS is missing DEVIL_ prefix", () => {
      const reports = runRule("src/components/button/button.tsx", [
        "BUTTON_VARIANTS", // Missing DEVIL_ prefix
        "DEVIL_BUTTON_DEFAULT_VARIANTS",
      ]);
    ...
    it("should fail when DEFAULT_VARIANTS is missing DEVIL_ prefix", () => {
      const reports = runRule("src/components/button/button.tsx", [
        "DEVIL_BUTTON_VARIANTS",
        "BUTTON_DEFAULT_VARIANTS", // Missing DEVIL_ prefix
      ]);
    ...
    it("should fail when BASE_STYLES is missing DEVIL_ prefix", () => {
      const reports = runRule("src/components/button/button.tsx", [
        "DEVIL_BUTTON_VARIANTS",
        "DEVIL_BUTTON_DEFAULT_VARIANTS",
        "BUTTON_BASE_STYLES", // Missing DEVIL_ prefix
      ]);

      expect(reports).toHaveLength(1);
      expect(reports[0].messageId).toBe("incorrectName");
      expect(reports[0].data!.actual).toBe("BUTTON_BASE_STYLES");
      expect(reports[0].data!.expected).toBe("DEVIL_BUTTON_BASE_STYLES");
    });
  });

  describe("Invalid exports - Missing required exports", () => {
    it("should fail when DEVIL_BUTTON_VARIANTS is missing", () => {
      const reports = runRule("src/components/button/button.tsx", [
        "DEVIL_BUTTON_DEFAULT_VARIANTS",
      ]);
      ...
      expect(reports[0].messageId).toBe("missingVariants");
      expect(reports[0].data!.component).toBe("BUTTON");
      expect(reports[0].data!.found).toContain("DEVIL_BUTTON_DEFAULT_VARIANTS");
    });

    it("should fail when DEVIL_BUTTON_DEFAULT_VARIANTS is missing", () => {
      const reports = runRule("src/components/button/button.tsx", [
        "DEVIL_BUTTON_VARIANTS",
      ]);
      ...
      expect(reports[0].messageId).toBe("missingDefaultVariants");
      expect(reports[0].data!.component).toBe("BUTTON");
      expect(reports[0].data!.found).toContain("DEVIL_BUTTON_VARIANTS");
    });
  ...
  describe("Invalid exports - Name mismatch", () => {
    it("should fail when DEVIL_BUTTON_VARIANTS is used in badge.tsx", () => {
      const reports = runRule("src/components/badge/badge.tsx", [
        "DEVIL_BUTTON_VARIANTS", // Wrong component name
        "DEVIL_BADGE_DEFAULT_VARIANTS",
      ]);

      expect(reports).toHaveLength(2); // incorrectName + missingVariants
      expect(reports[0].messageId).toBe("incorrectName");
      expect(reports[0].data!.actual).toBe("DEVIL_BUTTON_VARIANTS");
      expect(reports[0].data!.expected).toBe("DEVIL_BADGE_VARIANTS");
      expect(reports[1].messageId).toBe("missingVariants");
    });

    it("should fail when DEVIL_INPUT_DEFAULT_VARIANTS is used in button.tsx", () => {
      const reports = runRule("src/components/button/button.tsx", [
        "DEVIL_BUTTON_VARIANTS",
        "DEVIL_INPUT_DEFAULT_VARIANTS", // Wrong component name
      ]);

      expect(reports).toHaveLength(2); // incorrectName + missingDefaultVariants
      expect(reports[0].messageId).toBe("incorrectName");
      expect(reports[0].data!.actual).toBe("DEVIL_INPUT_DEFAULT_VARIANTS");
      // Note: Due to rule checking _VARIANTS before _DEFAULT_VARIANTS,
      // this incorrectly suggests DEVIL_BUTTON_VARIANTS instead of DEVIL_BUTTON_DEFAULT_VARIANTS
      expect(reports[0].data!.expected).toBe("DEVIL_BUTTON_VARIANTS");
      expect(reports[1].messageId).toBe("missingDefaultVariants");
    });

    it("should fail when DEVIL_DIALOG_BASE_STYLES is used in button.tsx", () => {
      const reports = runRule("src/components/button/button.tsx", [
        "DEVIL_BUTTON_VARIANTS",
        "DEVIL_BUTTON_DEFAULT_VARIANTS",
        "DEVIL_DIALOG_BASE_STYLES", // Wrong component name
      ]);

      expect(reports).toHaveLength(1);
      expect(reports[0].messageId).toBe("incorrectName");
      expect(reports[0].data!.actual).toBe("DEVIL_DIALOG_BASE_STYLES");
      expect(reports[0].data!.expected).toBe("DEVIL_BUTTON_BASE_STYLES");
    });
  });
  ...
  describe("Edge cases", () => {
    it("should handle multiple incorrect exports", () => {
      const reports = runRule("src/components/button/button.tsx", [
        "BUTTON_VARIANTS", // Missing DEVIL_ prefix
        "DEVIL_INPUT_DEFAULT_VARIANTS", // Wrong component name
        "DIALOG_BASE_STYLES", // Missing DEVIL_ prefix
      ]);
    ...
    it("should handle kebab-case to UPPER_SNAKE_CASE conversion", () => {
      const reports = runRule(
        ...
        [
          "DEVIL_CLIPBOARD_TEXT_VARIANTS",
          "DEVIL_CLIPBOARD_TEXT_DEFAULT_VARIANTS",
        ],
      );
  ...
  describe("Message formatting", () => {
    it("should provide helpful error messages for missing VARIANTS", () => {
      ...
      expect(reports[0].data).toEqual({
        component: "BUTTON",
        found: "DEVIL_BUTTON_DEFAULT_VARIANTS",
      });
    });
    ...
    it("should provide helpful error messages for incorrect names", () => {
      ...
      expect(reports[0].data).toEqual({
        ...
        expected: "DEVIL_BUTTON_BASE_STYLES",
      });
    });
