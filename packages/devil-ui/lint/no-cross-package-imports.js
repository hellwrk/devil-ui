 ...
 * like packages (e.g., ./devil-ui/ or ../devil-ui/ within the same package).
 */
...
function getImportSource(node) {
  // Static import: import x from "../devil-ui/foo"
  if (node.source && node.source.type === "Literal") {
    return node.source.value;
  }
  // Dynamic import: import("../devil-ui/foo")
  if (
...
export const noCrossPackageImportsRule = defineRule({
  meta: {
    ...
    messages: {
      [RULE_NAME]:
        "Cross-package relative import detected. Import from '{{packageName}}' using its package name instead of relative paths ('{{importPath}}').",
    },
    schema: [],
  },
  defaultOptions: [],
  createOnce(context) {
    ...
    return {
      // Static imports: import x from "../devil-ui/foo"
      ...
      // Dynamic imports: import("../devil-ui/foo")
      ...
      // require() calls: require("../devil-ui/foo")
      CallExpression(node) {
        if (
      ...
      // export from: export { x } from "../devil-ui/foo"
