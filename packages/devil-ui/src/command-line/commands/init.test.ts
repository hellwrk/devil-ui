...
describe("init command integration", () => {
  ...
  beforeEach(() => {
    ...
    testDir = join(tmpdir(), `devil-init-test-${Date.now()}`);
  ...
  describe("config file creation", () => {
    it("creates devil-ui.json with correct structure", () => {
  ...
  describe("config validation", () => {
    it("detects existing config files", () => {
      ...
      // Create config
      writeConfig(
        { blocksDir: "src/components/devil-ui", version: "1.0.0" },
        testDir,
      );
    ...
    it("handles missing config directory gracefully", () => {
      const nonExistentDir = join(tmpdir(), "does-not-exist-devil-test");
