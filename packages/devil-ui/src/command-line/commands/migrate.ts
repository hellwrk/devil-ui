/**
 * Devil CLI migrate command
 *
 * For CONSUMERS: Exports the token rename map when Devil releases breaking changes.
 * The actual rename map is embedded at build time from the theme-generator config.
 *
 * For DEVIL MAINTAINERS: Use `pnpm migrate:tokens` for the full migration workflow.
 */

// This map should be updated when devil releases a breaking change to token names.
...
const MIGRATE_HELP = `
Devil Token Migration Tool (for consumers)

When Devil releases breaking changes to token names, use this command
to get the rename map for updating your codebase.

USAGE:
  npx @hellwrk/devil-ui migrate [options]

OPTIONS:
  --json         Output machine-readable JSON (default)
  --classes      Output as class-level mapping (bg-devil-base -> bg-devil-base)
  --to-old       Reverse mapping (new names -> old names)
  --help         Show this help message

EXAMPLES:
  # Check if there are pending migrations
  npx @hellwrk/devil-ui migrate

  # Get class-level mapping for sed/find-replace
  npx @hellwrk/devil-ui migrate --classes

  # Get JSON for custom codemod
  npx @hellwrk/devil-ui migrate --json > rename-map.json

USING WITH sed:
  npx @hellwrk/devil-ui migrate --classes | grep "^bg-\\|^text-" | \\
...
export function migrate(args: string[]): void {
  ...
  if (!hasPendingMigrations()) {
    ...
    console.log("\nYour codebase is up to date with the current Devil version.");
  ...
  if (outputClasses) {
    ...
    console.log(`# Devil Token Migration (${direction})`);
  ...
  const output = {
    meta: {
      description: "Devil token class name migration map",
      direction,
