...
describe("add command - integration tests", () => {
  ...
  beforeEach(() => {
    ...
    tempDir = mkdtempSync(join(tmpdir(), "devil-add-test-"));
  });
  ...
  describe("error handling", () => {
    ...
    it("should require devil-ui.json to exist", async () => {
      const { add } = await import("./add.js");
      const exitSpy = vi.spyOn(process, "exit").mockImplementation(() => {
        throw new Error("process.exit called");
      });
    ...
    it("should validate block exists in registry", async () => {
      ...
      // Create devil-ui.json in temp directory
      const devilConfig = {
        blocksDir: "src/components/devil-ui",
        version: "1.0.0",
      };
      writeFileSync(
        join(tempDir, "devil-ui.json"),
        JSON.stringify(devilConfig, null, 2),
      );
  ...
  describe("import transformation", () => {
    it("should transform component imports", () => {
      ...
      expect(output).toBe('import { Tabs } from "@hellwrk/devil-ui";');
    });

    it("should transform util imports", () => {
      const input = 'import { cn } from "../../utils/cn";';
      const output = transformImports(input);
      expect(output).toBe('import { cn } from "@hellwrk/devil-ui";');
    });
    ...
    it("should consolidate mixed value and type imports using inline type syntax", () => {
      ...
      expect(output).toBe(
        'import { Tabs, type TabsItem } from "@hellwrk/devil-ui";',
      );
    });
    ...
    it("should convert type-only imports to inline type syntax", () => {
      ...
      const output = transformImports(input);
      // Uses inline type syntax to satisfy import/no-duplicates with prefer-inline: true
      expect(output).toBe('import { type TabsItem } from "@hellwrk/devil-ui";');
    });
    ...
    it("should consolidate real PageHeader imports into a single devil import", () => {
      const input = `import { ReactNode } from "react";
import { Tabs, type TabsItem } from "../../components/tabs";
import { cn } from "../../utils/cn";`;
      ...
      // All devil imports should be consolidated with inline type syntax
  ...
  describe("transformer integration", () => {
    it("should transform real PageHeader file content correctly", () => {
      ...
      expect(transformed).toContain("@hellwrk/devil-ui");
    ...
    it("should consolidate multiple imports into a single devil import", () => {
      const input = `import { ReactNode } from "react";
...
import { useState } from "react";`;
      ...
      // All Devil imports should be consolidated into a single import
      expect(output).toContain(
        'import { Tabs, Button, cn } from "@hellwrk/devil-ui"',
      );
