#!/usr/bin/env node
/**
 * Add a Devil block to the project
...
export async function add(blockName: string | undefined): Promise<void> {
  ...
  if (!config) {
    console.error(
      "Error: devil-ui.json not found. Run 'devil init' first to initialize the configuration.",
    );
    process.exit(1);
  }
  ...
  if (block.dependencies.length > 0) {
    console.log(`\n📚 This block depends on the following Devil components:\n`);
    for (const dep of block.dependencies) {
      console.log(`  - ${dep} (from devil-ui)`);
    }
    console.log("\nMake sure devil-ui is installed in your project:");
    console.log("  pnpm add @hellwrk/devil-ui");
  }
