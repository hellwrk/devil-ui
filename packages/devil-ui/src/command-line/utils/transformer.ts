/**
 * Import path transformer for Devil blocks
...
/**
 * Checks if an import path should be transformed to devil-ui
 */
function shouldTransformToDevil(path: string): boolean {
  if (!path.startsWith("../")) {
    return false;
  }
...
function buildConsolidatedImport(
  ...
  return `import { ${parts.join(", ")} } from "@hellwrk/devil-ui";`;
}

/**
 * Transforms relative imports in block source code to package imports
 *
 * This function consolidates all imports from devil-ui into a single
 ...
 * - `../../components/tabs` + `../../utils/cn` → `import { Tabs, cn } from "@hellwrk/devil-ui";`
 * - Mixed value/type imports → `import { Button, type ButtonProps } from "@hellwrk/devil-ui";`
 *
...
export function transformImports(content: string): string {
  ...
  // Separate devil imports from non-devil imports
  const devilImports: ParsedImport[] = [];
  const nonDevilImports: ParsedImport[] = [];

  for (const imp of parsedImports) {
    if (shouldTransformToDevil(imp.path)) {
      devilImports.push(imp);
    } else {
      nonDevilImports.push(imp);
    }
  }

  // If no devil imports, return unchanged
  if (devilImports.length === 0) {
    return content;
  }

  // Collect all value and type imports from devil imports
  const allValueImports: string[] = [];
  const allTypeImports: string[] = [];

  for (const imp of devilImports) {
  ...
  // Replace the first devil import with the consolidated import
  // and remove all other devil imports
  let result = content;

  // Process in reverse order to preserve indices
  const sortedDevilImports = [...devilImports].sort(
    (a, b) => b.startIndex - a.startIndex,
  );

  for (let i = 0; i < sortedDevilImports.length; i++) {
    const imp = sortedDevilImports[i];
    if (i === sortedDevilImports.length - 1) {
