...
export const DEVIL_METER_VARIANTS = {
  // Meter currently has no variant options but structure is ready for future additions
} as const;

export const DEVIL_METER_DEFAULT_VARIANTS = {} as const;

// Derived types from DEVIL_METER_VARIANTS
export interface DevilMeterVariantsProps {}

export function meterVariants(_props: DevilMeterVariantsProps = {}) {
  return cn(
    // Base styles
 ...
 * ```
 */
export interface MeterProps extends RootProps, DevilMeterVariantsProps {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-4">
        <BaseMeter.Label className="text-xs text-devil-subtle">
          {label}
        </BaseMeter.Label>
        {customValue ? (
          <span className="text-sm font-medium text-devil-default tabular-nums">
            {customValue}
          </span>
        ) : (
          <>
            {showValue && (
              <BaseMeter.Value className="text-sm font-medium text-devil-default tabular-nums" />
            )}
          </>
        )}
      </div>
      <BaseMeter.Track
        className={cn(
          "relative h-2 w-full overflow-hidden rounded-full bg-devil-fill",
          trackClassName,
        )}
      >
        <BaseMeter.Indicator
          className={cn(
            "absolute inset-y-0 left-0 rounded-full bg-linear-to-r from-devil-brand via-devil-brand to-devil-brand transition-[width] duration-300 ease-out",
            indicatorClassName,
          )}
        />
      </BaseMeter.Track>
