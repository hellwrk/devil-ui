...
import {
  ...
  type DevilInputSize,
  type DevilInputVariant,
} from "../input/input";
import { Field, type FieldErrorMatch } from "../field/field";

export const DEVIL_SENSITIVE_INPUT_VARIANTS = DEVIL_INPUT_VARIANTS;

export const DEVIL_SENSITIVE_INPUT_DEFAULT_VARIANTS = {
  size: "base",
  variant: "default",
} as const;
  ...
  (
    {
      ...
      onValueChange,
      onCopy,
      size = DEVIL_SENSITIVE_INPUT_DEFAULT_VARIANTS.size,
      variant: variantProp,
      disabled = false,
      readOnly = false,
      id,
    ...
    if (process.env.NODE_ENV !== "production" && variantProp === "error") {
      console.warn(
        '[Devil SensitiveInput]: variant="error" is deprecated. ' +
    ...
    const containerClassName = cn(
      ...
      "focus-within:outline-2 focus-within:outline-devil-focus",
    ...
    const containerContent = (
      <>
        ...
        <BaseInput
          ...
          className={cn(
            "devil-input-placeholder w-full border-0 bg-transparent p-0 text-devil-default ring-0 outline-none disabled:cursor-not-allowed disabled:text-devil-subtle",
            size === "xs" && "pr-5",
            size === "sm" && "pr-6",
            size === "base" && "pr-8",
            size === "lg" && "pr-10",
        ...
        <span
          className={cn(
            ...
            // Text color - use text-devil-default to contrast with bg-devil-control input background
            "text-devil-default",
        ...
        >
          ...
          <span className="relative">
            ...
            {isMaskedWithValue && !disabled && (
              <span className="invisible absolute top-0 left-0 whitespace-nowrap text-devil-subtle group-focus-within/container:visible group-hover/mask:visible">
                Click to reveal
              </span>
            )}
          </span>
        ...
        <button
          type="button"
          data-devil-component="SensitiveInput"
          data-devil-part="toggle-visibility"
          onClick={handleToggleVisibility}
          onKeyDown={(e) => e.stopPropagation()}
          aria-label={mode === "revealed" ? "Hide value" : "Reveal value"}
          tabIndex={showEyeButton ? 0 : -1}
          className={cn(
            "absolute top-1/2 right-0 -translate-y-1/2 cursor-pointer text-devil-subtle hover:text-devil-default focus:text-devil-default focus:ring-devil-focus/50 focus-visible:rounded-sm focus-visible:ring-2 focus-visible:ring-devil-brand",
        ...
        {hasValue && !disabled && (
          <button
            type="button"
            data-devil-component="SensitiveInput"
            data-devil-part="copy"
            onClick={copyToClipboard}
            onKeyDown={(e) => e.stopPropagation()}
            aria-label={copied ? "Copied" : "Copy to clipboard"}
            className={cn(
              "absolute -top-px right-2 -translate-y-full cursor-pointer rounded-t-md bg-devil-brand px-2 py-0.5 text-xs text-white opacity-0 transition-opacity group-focus-within/container:opacity-100 group-hover/container:opacity-100 hover:brightness-120 focus:ring-devil-focus/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-devil-brand",
      ...
      <div>
        {isMaskedWithValue ? (
          <div
            ...
            role="button"
            data-devil-component="SensitiveInput"
            data-devil-part="masked-container"
