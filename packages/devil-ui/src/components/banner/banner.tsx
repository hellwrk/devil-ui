...
export const DEVIL_BANNER_DEFAULT_VARIANTS = {
  variant: "default",
  size: "base",
} as const;

// Derived types from DEVIL_BANNER_VARIANTS
export type DevilBannerVariant = keyof typeof DEVIL_BANNER_VARIANTS.variant;
export type DevilBannerSize = keyof typeof DEVIL_BANNER_VARIANTS.size;
...
export interface DevilBannerVariantsProps {
  /**
   ...
   * @default "base"
   */
  size?: DevilBannerSize;
}

export function bannerVariants({
  variant = DEVIL_BANNER_DEFAULT_VARIANTS.variant,
  size = DEVIL_BANNER_DEFAULT_VARIANTS.size,
}: DevilBannerVariantsProps = {}) {
  const resolvedVariant = resolveVariant(
    DEVIL_BANNER_VARIANTS.variant,
    variant,
    DEVIL_BANNER_DEFAULT_VARIANTS.variant,
  );
  const resolvedSize = resolveVariant(
    DEVIL_BANNER_VARIANTS.size,
    size,
    DEVIL_BANNER_DEFAULT_VARIANTS.size,
  );
  ...
  return cn(
    ...
    // Apply size styles (spacing / radius / text) from DEVIL_BANNER_VARIANTS
...
> {
  ...
  /**
   * Action slot for a CTA button or link. Compact banners render a Devil `Link`
   ...
   * renders a Devil `Link` action inline with the description, and sets its
                ...
                {title && (
                  <span className="leading-snug font-medium">
                    ...
                    {!description && hasInlineLinkAction && (
                      <span className="ml-1.5 [&_[data-devil-component=Link]]:inline">
                        {action}
                      </span>
                    )}
                  </span>
