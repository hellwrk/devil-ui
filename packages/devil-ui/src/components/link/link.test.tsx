...
import { Link, DEVIL_LINK_VARIANTS, linkVariants } from "./link";
...
describe("Link", () => {
  ...
  it("should apply inline variant classes", () => {
    expect(DEVIL_LINK_VARIANTS.variant.inline.classes).toContain(
      "text-devil-link",
    );
    expect(DEVIL_LINK_VARIANTS.variant.inline.classes).toContain("underline");
    expect(DEVIL_LINK_VARIANTS.variant.inline.classes).toContain("link-current");
  });

  it("should apply current variant classes", () => {
    expect(DEVIL_LINK_VARIANTS.variant.current.classes).toContain(
      "text-current",
    );
    expect(DEVIL_LINK_VARIANTS.variant.current.classes).toContain("underline");
    expect(DEVIL_LINK_VARIANTS.variant.current.classes).toContain(
      "link-current",
    );
  });
  ...
  it("should apply plain variant classes", () => {
    expect(DEVIL_LINK_VARIANTS.variant.plain.classes).toContain(
      "text-devil-link",
    );
    expect(DEVIL_LINK_VARIANTS.variant.plain.classes).not.toContain("underline");
  });
  ...
  it("should generate variant classes via linkVariants helper", () => {
    expect(linkVariants({ variant: "inline" })).toContain("text-devil-link");
    expect(linkVariants({ variant: "current" })).toContain("text-current");
    expect(linkVariants({ variant: "plain" })).toContain("text-devil-link");
  });

  it("should default to inline variant", () => {
    expect(linkVariants()).toContain("text-devil-link");
...
describe("LinkProvider", () => {
  ...
  it("the custom LinkProvider component receives href for external URLs", () => {
     ...
     * in the LinkProvider wrapper — not in Devil's Link component.
     */
...
describe("LinkProvider integration patterns", () => {
  it("a well-written wrapper maps href to the router's navigation prop", () => {
     ...
     * This keeps Devil framework-agnostic — routing logic stays in the app.
     */
  ...
  it("a wrapper that only reads `to` will not receive the destination from href", () => {
     ...
     *
     * The solution is NOT to add `to` to Devil's Link API. Instead, the
