...
export const DEVIL_LINK_VARIANTS = {
  variant: {
    inline: {
      classes:
        // text-devil-link provides defensive color that won't be overridden by global `a` styles
        "text-devil-link underline underline-offset-[0.15em] decoration-[0.0625em] link-current transition-colors",
    ...
    plain: {
      classes:
        // text-devil-link provides defensive color that won't be overridden by global `a` styles
        "text-devil-link hover:text-devil-link/70 transition-colors",
      description: "Link without underline decoration",
    },
  },
} as const;

export const DEVIL_LINK_DEFAULT_VARIANTS = {
  variant: "inline",
} as const;

export type DevilLinkVariant = keyof typeof DEVIL_LINK_VARIANTS.variant;

export interface DevilLinkVariantsProps {
  /**
   ...
   * @default "inline"
   */
  variant?: DevilLinkVariant;
}

export function linkVariants({
  variant = DEVIL_LINK_DEFAULT_VARIANTS.variant,
}: DevilLinkVariantsProps = {}) {
  return cn(
    resolveVariant(
      DEVIL_LINK_VARIANTS.variant,
      variant,
      DEVIL_LINK_DEFAULT_VARIANTS.variant,
    ).classes,
  );
}
