...
import { Badge, badgeVariants, DEVIL_BADGE_VARIANTS } from "./badge";
...
describe("Badge", () => {
  ...
  describe("filled appearance (default)", () => {
    it("applies variant classes for filled badges", () => {
      ...
      expect(el.className).toContain("bg-devil-danger-tint");
    });
  ...
  describe("dot appearance", () => {
    it("renders a dot indicator for supported variants", () => {
      ...
      expect(dot!.className).toContain("bg-devil-success");
    });
    ...
    it("applies dot appearance classes instead of variant classes", () => {
      ...
      expect(badge.className).toContain("text-devil-default");
      // Should NOT contain the filled error classes
      expect(badge.className).not.toContain("bg-devil-danger-tint");
    });

    it("renders correct dot color per variant", () => {
      const cases = [
        { variant: "success" as const, expected: "bg-devil-success" },
        { variant: "warning" as const, expected: "bg-devil-badge-orange" },
        { variant: "error" as const, expected: "bg-devil-badge-red" },
        { variant: "neutral" as const, expected: "bg-devil-badge-neutral" },
      ];
...
describe("DEVIL_BADGE_VARIANTS", () => {
  it("has variant, appearance, and dotColor dimensions", () => {
    expect(DEVIL_BADGE_VARIANTS.variant).toBeDefined();
    expect(DEVIL_BADGE_VARIANTS.appearance).toBeDefined();
    expect(DEVIL_BADGE_VARIANTS.dotColor).toBeDefined();
  });

  it("every variant entry has classes and description", () => {
    for (const [dim, entries] of Object.entries(DEVIL_BADGE_VARIANTS)) {
