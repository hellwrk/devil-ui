...
export const DEVIL_FIELD_VARIANTS = {
  // Field currently has no variant options but structure is ready for future additions
} as const;

export const DEVIL_FIELD_DEFAULT_VARIANTS = {} as const;

// Derived types from DEVIL_FIELD_VARIANTS
export interface DevilFieldVariantsProps {
  /**
 ...
 * ```
 */
export interface FieldProps extends DevilFieldVariantsProps {
...
export function Field({
  ...
  return (
    <FieldBase.Root className={fieldVariants({ controlFirst })}>
      {!hideLabel && (
        <FieldBase.Label className="m-0 text-base font-medium text-devil-default select-none">
      ...
      {error ? (
        <FieldBase.Error
          className={cn(
            "text-sm leading-snug text-devil-danger",
        ...
        description && (
          <FieldBase.Description
            className={cn(
              "text-sm leading-snug text-devil-subtle",
              // Span full width in horizontal layout
              "col-span-full",
            )}
          >
