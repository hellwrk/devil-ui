...
import { inputVariants, DEVIL_INPUT_VARIANTS } from "../input/input";
...
export const DEVIL_AUTOCOMPLETE_VARIANTS = {
  size: DEVIL_INPUT_VARIANTS.size,
} as const;

export const DEVIL_AUTOCOMPLETE_DEFAULT_VARIANTS = {
  size: "base",
} as const;

// Derived types from DEVIL_AUTOCOMPLETE_VARIANTS
export type DevilAutocompleteSize = keyof typeof DEVIL_AUTOCOMPLETE_VARIANTS.size;

export interface DevilAutocompleteVariantsProps {
  /**
...
export function autocompleteVariants({
  size = DEVIL_AUTOCOMPLETE_DEFAULT_VARIANTS.size,
}: DevilAutocompleteVariantsProps = {}) {
  return cn(
    resolveVariant(
      DEVIL_INPUT_VARIANTS.size,
      size,
      DEVIL_AUTOCOMPLETE_DEFAULT_VARIANTS.size,
    ).classes,
  );
}
        ...
        <AutocompleteBase.Popup
          className={(state: AutocompleteBase.Popup.State) =>
            cn(
              ...
              "bg-devil-control text-devil-default",
...
function Item({ children, ...props }: AutocompleteBase.Item.Props) {
  return (
    <AutocompleteBase.Item
      data-devil-component="Autocomplete"
      data-devil-part="item"
      {...props}
      className="group mx-1.5 grid cursor-pointer grid-cols-[1fr_16px] gap-2 rounded px-2 py-1.5 text-base data-highlighted:bg-devil-overlay data-selected:font-medium"
    >
...
function GroupLabel(props: AutocompleteBase.GroupLabel.Props) {
  return (
    <AutocompleteBase.GroupLabel
      {...props}
      className={cn(
        "mx-1.5 px-2 py-1.5 text-sm text-devil-strong",
        props.className,
      )}
    />
  );
...
function Group(props: AutocompleteBase.Group.Props) {
  return (
    <AutocompleteBase.Group
      {...props}
      className="mt-2 border-t border-devil-line pt-2 first:mt-0 first:border-t-0 first:pt-0"
    />
  );
}
...
function Separator(props: AutocompleteBase.Separator.Props) {
  return (
    <AutocompleteBase.Separator
      {...props}
      className={cn("mx-0 my-1 h-px bg-devil-line", props.className)}
    />
  );
}
