...
export const DEVIL_INPUT_VARIANTS = {
  size: {
    xs: {
  ...
  variant: {
    default: {
      classes: "focus:ring-devil-focus/50 focus:ring-[1.5px]",
      description: "Default input appearance",
    },
    error: {
      classes: "!ring-devil-danger focus:ring-devil-danger/50 focus:ring-[1.5px]",
      description: "Error state for validation failures",
    },
  },
} as const;

export const DEVIL_INPUT_DEFAULT_VARIANTS = {
  size: "base",
  variant: "default",
} as const;

export const DEVIL_INPUT_STYLING = {
  dimensions: {
...
// Derived types from DEVIL_INPUT_VARIANTS
export type DevilInputSize = keyof typeof DEVIL_INPUT_VARIANTS.size;
export type DevilInputVariant = keyof typeof DEVIL_INPUT_VARIANTS.variant;

export interface DevilInputVariantsProps {
  /**
   * Input size.
...
export function inputVariants({
  variant = DEVIL_INPUT_DEFAULT_VARIANTS.variant,
  size = DEVIL_INPUT_DEFAULT_VARIANTS.size,
  parentFocusIndicator = false,
  focusIndicator = false,
}: DevilInputVariantsProps = {}) {
  return cn(
    // Base styles
    "border-0 bg-devil-control text-devil-default ring ring-devil-line outline-none focus:outline-none",
    // Disabled state and placeholder styles (using vanilla CSS class for Chrome compatibility)
    "devil-input-placeholder disabled:text-devil-disabled",
    // Apply size styles from DEVIL_INPUT_VARIANTS
    resolveVariant(
      DEVIL_INPUT_VARIANTS.size,
      size,
      DEVIL_INPUT_DEFAULT_VARIANTS.size,
    ).classes,
    // Apply variant styles from DEVIL_INPUT_VARIANTS
    resolveVariant(
      DEVIL_INPUT_VARIANTS.variant,
      variant,
      DEVIL_INPUT_DEFAULT_VARIANTS.variant,
    ).classes,
    // Focus state handling
    parentFocusIndicator &&
      (variant === "error"
        ? "focus-within:ring-[1.5px] focus-within:ring-devil-danger/50"
        : "focus-within:ring-[1.5px] focus-within:ring-devil-focus/50"),
    focusIndicator &&
      (variant === "error"
        ? "focus:ring-[1.5px] focus:ring-devil-danger/50"
        : "focus:ring-[1.5px] focus:ring-devil-focus/50"),
  );
}
...
export const Input = forwardRef<HTMLInputElement, InputProps>((props, ref) => {
  ...
  if (process.env.NODE_ENV !== "production" && variantProp === "error") {
    console.warn(
      '[Devil Input]: variant="error" is deprecated. ' +
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    if (!hasLabel && !hasAriaLabel && !hasAriaLabelledBy) {
      console.warn(
        "[Devil Input]: Input must have an accessible name. Provide either:\n" +
 ...
 * />
 */
export type InputProps = Pick<DevilInputVariantsProps, "size" | "variant"> &
  BaseInputProps & {
