...
export const DEVIL_GRID_VARIANTS = {
  variant: {
    "2up": {
...
export const DEVIL_GRID_DEFAULT_VARIANTS = {
  gap: "base",
} as const;

export type DevilGridVariant = keyof typeof DEVIL_GRID_VARIANTS.variant;
export type DevilGridGap = keyof typeof DEVIL_GRID_VARIANTS.gap;
...
export function gridVariants({
  variant,
  gap = DEVIL_GRID_DEFAULT_VARIANTS.gap,
}: {
  variant?: DevilGridVariant;
  gap?: DevilGridGap;
} = {}) {
  return cn(
    "grid",
    variant &&
      resolveVariant(DEVIL_GRID_VARIANTS.variant, variant, "2up").classes,
    resolveVariant(DEVIL_GRID_VARIANTS.gap, gap, DEVIL_GRID_DEFAULT_VARIANTS.gap)
      .classes,
  );
}
...
export function gridItemVariants({
  ...
  return cn(
    mobileDivider &&
      variant === "4up" &&
      "border-b border-devil-hairline pb-8 md:border-b-0 md:pb-0",
  );
}
