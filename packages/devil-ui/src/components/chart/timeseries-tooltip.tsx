  ...
  return (
    <>
      {state.type === "marker" ? (
        ...
        <>
          <div className="mb-1 text-xs font-semibold text-devil-default">
...
function MarkerTooltipContent({
  ...
  return (
    <>
      ...
      <div className="space-y-1">
        {state.markers.map((marker, index) => (
          <div key={`${marker.timestamp}-${marker.label}-${index}`}>
            <div className="flex items-center gap-2 text-xs text-devil-default">
              <span
            ...
            {marker.description && (
              <div className="mt-0.5 ml-5 text-xs text-devil-default">
                {marker.description}
              </div>
            )}
          </div>
      ...
      {state.rows.length > 0 && (
        <div className="mt-2 border-t border-devil-line pt-2">
          <SeriesTooltipRows
            rows={state.rows}
            hiddenCount={state.hiddenCount}
            formatValue={formatValue}
...
function SeriesTooltipRows({
  ...
  return (
    <>
      {rows.map((row) => (
        ...
        >
          <div className="flex min-w-0 items-center gap-2">
            ...
            />
            <span
              className="truncate text-xs font-medium text-devil-default"
              title={row.name}
            >
              {row.name}
            </span>
          </div>
          <span className="shrink-0 text-xs font-semibold text-devil-default">
            {formatValue
      ...
      {hiddenCount > 0 && (
        <div className="mt-1 text-xs text-devil-subtle">+{hiddenCount} more</div>
      )}
    </>
  );
}
