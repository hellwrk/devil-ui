...
import { Chart, type ChartEvents, type DevilChartOption } from "./EChart";
...
function getMapName(geoJson: MapGeoJson, mapName?: string) {
  ...
  const generated = `devil-map-${hashString(JSON.stringify(geoJson))}`;
...
function BubbleMapRoot<T>(
  ...
  const options = useMemo<DevilChartOption>(() => {
    ...
    return {
      ...
      tooltip: showTooltip
        ? {
            trigger: "item",
            triggerOn: "mousemove",
            backgroundColor: "var(--color-devil-base)",
            borderColor: "var(--color-devil-line)",
            borderWidth: 1,
            padding: 8,
            textStyle: {
              color: "var(--text-color-devil-default)",
              fontSize: 13,
            },
            ...
            dangerousHtmlFormatter: (params: unknown) => {
              ...
              const valueStr =
                v !== undefined
                  ? `<span style="color:var(--text-color-devil-subtle)">${escapeHtml(valueFormat(v))}</span>`
                  : "";
...
export interface ChoroplethMapProps<T> {
  ...
  /**
   * Sequential colour ramp (low → high). Defaults to the Devil choropleth blues,
