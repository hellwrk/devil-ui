...
export const DEVIL_POPOVER_VARIANTS = {
  side: {
    top: {
      classes: "",
...
export const DEVIL_POPOVER_DEFAULT_VARIANTS = {
  side: "bottom",
} as const;

// Derived types from DEVIL_POPOVER_VARIANTS
export type DevilPopoverSide = keyof typeof DEVIL_POPOVER_VARIANTS.side;

export interface DevilPopoverVariantsProps {
  /**
 ...
 * ```
 */
export type PopoverContentProps = DevilPopoverVariantsProps & {
  /**
...
function PopoverContent({
  children,
  side = DEVIL_POPOVER_DEFAULT_VARIANTS.side,
  align = "center",
  sideOffset = 8,
  alignOffset = 0,
  ...
  return (
    <PopoverBase.Portal container={container}>
      ...
      >
        <PopoverBase.Popup
          className={cn(
            "flex origin-(--transform-origin) flex-col rounded-lg bg-devil-base px-4 py-3 text-sm text-devil-default",
...
function PopoverDescription({ className, ...props }: PopoverDescriptionProps) {
  return (
    <PopoverBase.Description
      className={cn("m-0 text-base leading-6 text-devil-subtle", className)}
      {...props}
    />
  );
}
