...
export const DEVIL_LABEL_VARIANTS = {
  // Label currently has no variant options but structure is ready for future additions
} as const;

export const DEVIL_LABEL_DEFAULT_VARIANTS = {} as const;

// Derived types from DEVIL_LABEL_VARIANTS
export interface DevilLabelVariantsProps {}

export function labelVariants(_props: DevilLabelVariantsProps = {}) {
  return cn(
    ...
    "m-0 text-base font-medium text-devil-default",
  );
}
 ...
 * ```
 */
export interface LabelProps extends DevilLabelVariantsProps {
...
export function Label({
  ...
  const content = (
    <>
      {children}
      {showOptional && (
        <span className="font-normal text-devil-subtle">(optional)</span>
      )}
      {tooltip && (
        <Tooltip
          content={tooltip}
