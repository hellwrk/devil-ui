...
export const DEVIL_TABLE_OF_CONTENTS_VARIANTS = {
  state: {
    default: {
      classes:
        "text-devil-subtle hover:border-devil-line hover:text-devil-default hover:font-medium",
      description: "Inactive section link",
    },
    active: {
      classes: "border-devil-brand font-medium text-devil-default",
      description: "Currently visible / active section",
    },
  },
} as const;

export const DEVIL_TABLE_OF_CONTENTS_DEFAULT_VARIANTS = {
  state: "default",
} as const;

export type DevilTableOfContentsState =
  keyof typeof DEVIL_TABLE_OF_CONTENTS_VARIANTS.state;
...
>(({ className, ...props }, ref) => (
  <p
    ref={ref}
    className={cn(
      "mb-3 text-xs font-semibold tracking-wide text-devil-subtle uppercase",
      className,
    )}
    {...props}
  />
...
>(({ className, ...props }, ref) => (
  <ul
    ref={ref}
    className={cn(
      "flex flex-col gap-2 border-l-2 border-devil-hairline",
      className,
    )}
    {...props}
  />
...
>(({ active = false, className, children, render, ...props }, ref) => {
  const stateClasses = active
    ? DEVIL_TABLE_OF_CONTENTS_VARIANTS.state.active.classes
    : DEVIL_TABLE_OF_CONTENTS_VARIANTS.state.default.classes;
  ...
  const sharedProps = {
    ...
    "data-devil-component": "TableOfContents",
...
const NESTED_UL_CLASSES =
  "flex flex-col gap-2 border-l-2 border-devil-hairline [&>li>a]:pl-7 [&>li>button]:pl-7";
  ...
  (
    ...
    return (
      <li
        ...
        {...props}
      >
        <p className="py-0.5 pl-4 text-sm leading-5 font-medium text-devil-subtle">
          {label}
        </p>
        <ul className={cn(NESTED_UL_CLASSES)}>{children}</ul>
      </li>
