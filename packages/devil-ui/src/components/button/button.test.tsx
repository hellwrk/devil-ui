...
describe("Button", () => {
  ...
  it("keeps emphasized variant rings color-matched when pressed or focused", () => {
    for (const variant of ["primary", "destructive"] as const) {
      ...
      expect(className).toContain("ring-(--devil-button-emphasis-ring)");
      expect(className).toContain("focus:ring-(--devil-button-emphasis-ring)");
      expect(className).toContain(
        "focus-visible:ring-(--devil-button-emphasis-ring)",
      );
      expect(className).toContain("active:ring-(--devil-button-emphasis-ring)");
      expect(className).not.toContain("focus:ring-devil-focus/50");
      expect(className).not.toContain("focus-visible:ring-devil-brand");
    }
  });
});
...
describe("LinkButton", () => {
  ...
  it("title prop wraps the anchor in a Tooltip and removes the native title attribute", () => {
    ...
    // title is intercepted by the Devil Tooltip wrapper, not set as a native attribute
  ...
  describe("disabled", () => {
    ...
    it("preserves the LinkButton data attribute", () => {
      ...
      const button = screen.getByRole("button", { name: "Home" });
      expect(button.getAttribute("data-devil-component")).toBe("LinkButton");
    });
