...
export const DEVIL_CHECKBOX_VARIANTS = {
  variant: {
    default: {
      classes:
        "[&:focus-within>span]:ring-devil-focus [&:hover>span]:ring-devil-hairline",
      description: "Default checkbox appearance",
    },
    error: {
      classes: "[&>span]:ring-devil-danger",
      description: "Error state for validation failures",
    },
  },
} as const;

export const DEVIL_CHECKBOX_DEFAULT_VARIANTS = {
  variant: "default",
} as const;

// Derived types from DEVIL_CHECKBOX_VARIANTS
export type DevilCheckboxVariant = keyof typeof DEVIL_CHECKBOX_VARIANTS.variant;

export interface DevilCheckboxVariantsProps {
  /**
   * Visual variant.
   ...
   * @default "default"
   */
  variant?: DevilCheckboxVariant;
}

export function checkboxVariants({
  variant = DEVIL_CHECKBOX_DEFAULT_VARIANTS.variant,
}: DevilCheckboxVariantsProps = {}) {
  return cn(
    resolveVariant(
      DEVIL_CHECKBOX_VARIANTS.variant,
      variant,
      DEVIL_CHECKBOX_DEFAULT_VARIANTS.variant,
    ).classes,
  );
}

// Legacy type alias for backwards compatibility
export type CheckboxVariant = DevilCheckboxVariant;
  ...
  (
    ...
    if (process.env.NODE_ENV !== "production") {
      ...
      if (!hasLabel && !hasAriaLabel && !hasAriaLabelledBy) {
        console.warn(
          "[Devil Checkbox]: Checkbox must have an accessible name. Provide either:\n" +
    ...
    const checkboxControl = (
      <BaseCheckbox.Root
        ...
        className={cn(
          "relative flex h-4 w-4 shrink-0 items-center justify-center rounded-sm border-0 bg-devil-base ring after:absolute after:-inset-x-3 after:-inset-y-2 focus:outline-none",
          label && "mt-0.5",
          variant === "error" ? "ring-devil-danger" : "ring-devil-hairline",
          !disabled &&
            "hover:ring-devil-hairline focus:ring-2 focus:ring-devil-focus focus-visible:ring-2 focus-visible:ring-devil-brand",
          "data-[checked]:bg-devil-contrast data-[checked]:ring-devil-contrast data-[indeterminate]:bg-devil-contrast data-[indeterminate]:ring-devil-contrast",
      ...
      >
        <BaseCheckbox.Indicator
          keepMounted
          className="flex items-center justify-center text-devil-inverse data-[unchecked]:invisible"
    ...
    return (
      ...
      >
        <BaseCheckbox.Root
          ...
          onCheckedChange={onCheckedChange}
          className={cn(
            "peer relative mt-0.5 flex h-4 w-4 shrink-0 items-center justify-center rounded-sm border-0 bg-devil-base ring after:absolute after:-inset-x-3 after:-inset-y-2",
            variant === "error" ? "ring-devil-danger" : "ring-devil-hairline",
            !disabled &&
              "group-hover:ring-devil-hairline hover:ring-devil-hairline focus:ring-2 focus:ring-devil-focus focus-visible:ring-2 focus-visible:ring-devil-brand",
            "data-[checked]:bg-devil-contrast data-[checked]:ring-devil-contrast data-[indeterminate]:bg-devil-contrast data-[indeterminate]:ring-devil-contrast",
          )}
        >
          <BaseCheckbox.Indicator
