...
export const DEVIL_LOADER_VARIANTS = {
  size: {
    sm: {
      value: 16,
...
export const DEVIL_LOADER_DEFAULT_VARIANTS = {
  size: "base",
} as const;

// Derived types from DEVIL_LOADER_VARIANTS
export type DevilLoaderSize = keyof typeof DEVIL_LOADER_VARIANTS.size;

export interface DevilLoaderVariantsProps {
  /**
   ...
   * @default "base"
   */
  size?: DevilLoaderSize | number;
}

export function loaderVariants({
  size = DEVIL_LOADER_DEFAULT_VARIANTS.size,
}: DevilLoaderVariantsProps = {}): number {
  if (typeof size === "number") return size;
  return resolveVariant(
    DEVIL_LOADER_VARIANTS.size,
    size,
    DEVIL_LOADER_DEFAULT_VARIANTS.size,
  ).value;
}
