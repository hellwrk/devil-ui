...
export const DEVIL_TOOLTIP_VARIANTS = {
  side: {
    top: {
      classes: "",
...
export const DEVIL_TOOLTIP_DEFAULT_VARIANTS = {
  side: "top",
} as const;

// Derived types from DEVIL_TOOLTIP_VARIANTS
export type DevilTooltipSide = keyof typeof DEVIL_TOOLTIP_VARIANTS.side;

export interface DevilTooltipVariantsProps {
  /**
   ...
   * @default "top"
   */
  side?: DevilTooltipSide;
}

export function tooltipVariants({
  side = DEVIL_TOOLTIP_DEFAULT_VARIANTS.side,
}: DevilTooltipVariantsProps = {}) {
  return cn(
    // Base styles
    "flex origin-[var(--transform-origin)] flex-col rounded-md bg-devil-base px-2.5 py-1.5 text-sm text-devil-default",
    "shadow-md outline-1 outline-devil-line",
    ...
    resolveVariant(
      DEVIL_TOOLTIP_VARIANTS.side,
      side,
      DEVIL_TOOLTIP_DEFAULT_VARIANTS.side,
    ).classes,
  );
}
