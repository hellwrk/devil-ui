...
export const DEVIL_BREADCRUMBS_VARIANTS = {
  size: {
    sm: {
...
export const DEVIL_BREADCRUMBS_DEFAULT_VARIANTS = {
  size: "base",
} as const;

export type DevilBreadcrumbsSize = keyof typeof DEVIL_BREADCRUMBS_VARIANTS.size;

export interface DevilBreadcrumbsVariantsProps {
  /**
   ...
   * @default "base"
   */
  size?: DevilBreadcrumbsSize;
}

export function breadcrumbsVariants({
  size = DEVIL_BREADCRUMBS_DEFAULT_VARIANTS.size,
}: DevilBreadcrumbsVariantsProps = {}) {
  return cn(
    "group mr-4 flex min-w-0 grow items-center overflow-hidden whitespace-nowrap",
    resolveVariant(
      DEVIL_BREADCRUMBS_VARIANTS.size,
      size,
      DEVIL_BREADCRUMBS_DEFAULT_VARIANTS.size,
    ).classes,
  );
}
...
const Link = ({
  ...
  return (
    <LinkComponent
      data-devil-component="Breadcrumbs"
      data-devil-part="link"
      to={href}
      ...
      className="flex shrink-0 items-center gap-1 whitespace-nowrap text-devil-subtle no-underline"
    >
...
function Separator() {
  return (
    <span
      className="flex shrink-0 items-center text-devil-inactive"
      aria-hidden="true"
    >
...
function MobileEllipsis() {
  return (
    <span className="flex shrink-0 items-center text-devil-subtle" aria-hidden>
      ...
    </span>
  );
}
...
function Clipboard({ text }: { text: string }) {
  ...
  return (
    ...
    >
      {isCopied ? (
        <CheckIcon weight="bold" className="text-devil-success" />
      ) : (
...
export interface BreadcrumbsProps
  extends PropsWithChildren, DevilBreadcrumbsVariantsProps {
...
function getMobileBreadcrumbChildren(children: ReactNode[]): ReactNode[] {
  ...
  const trailingItems: ReactNode[] = [
    <MobileEllipsis key="devil-breadcrumb-mobile-ellipsis" />,
    <Separator key="devil-breadcrumb-mobile-separator-leading" />,
    cloneElement(parentItem, { key: "devil-breadcrumb-mobile-parent" }),
    <Separator key="devil-breadcrumb-mobile-separator-trailing" />,
    cloneElement(currentItem, { key: "devil-breadcrumb-mobile-current" }),
  ];
