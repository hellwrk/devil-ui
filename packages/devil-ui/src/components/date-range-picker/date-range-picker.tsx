...
export const DEVIL_DATE_RANGE_PICKER_VARIANTS = {
  size: {
    sm: {
      classes: "p-3 gap-2",
      cellHeight: "h-[22px]",
  ...
  variant: {
    default: {
      classes: "bg-devil-overlay",
      description: "Default calendar appearance",
    },
    subtle: {
      classes: "bg-devil-base",
      description: "Subtle calendar with minimal background",
    },
  },
} as const;

export const DEVIL_DATE_RANGE_PICKER_DEFAULT_VARIANTS = {
  size: "base",
  variant: "default",
} as const;

// Derived types from DEVIL_DATE_RANGE_PICKER_VARIANTS
export type DevilDateRangePickerSize =
  keyof typeof DEVIL_DATE_RANGE_PICKER_VARIANTS.size;
export type DevilDateRangePickerVariant =
  keyof typeof DEVIL_DATE_RANGE_PICKER_VARIANTS.variant;

export interface DevilDateRangePickerVariantsProps {
  /**
   * Calendar size.
   ...
   * @default "default"
   */
  variant?: DevilDateRangePickerVariant;
}

export function dateRangePickerVariants({
  size = DEVIL_DATE_RANGE_PICKER_DEFAULT_VARIANTS.size,
  variant = DEVIL_DATE_RANGE_PICKER_DEFAULT_VARIANTS.variant,
}: DevilDateRangePickerVariantsProps = {}) {
  return cn(
    // Base styles
    "flex w-fit flex-col rounded-xl select-none",
    // Apply variant and size styles
    DEVIL_DATE_RANGE_PICKER_VARIANTS.variant[variant].classes,
    DEVIL_DATE_RANGE_PICKER_VARIANTS.size[size].classes,
  );
}

// Helper to get size config
function getSizeConfig(size: DevilDateRangePickerSize) {
  return DEVIL_DATE_RANGE_PICKER_VARIANTS.size[size];
}
 ...
 */
export interface DateRangePickerProps extends DevilDateRangePickerVariantsProps {
      ...
      <div className="flex gap-4">
        <div className={cn("relative", sizeConfig.calendarWidth)}>
          <button
            ...
            className="absolute top-0 left-0 cursor-pointer rounded bg-devil-interact/85 p-1.5 hover:bg-devil-interact"
        ...
        <div className={cn("relative", sizeConfig.calendarWidth)}>
          <button
            ...
            className="absolute top-0 right-0 cursor-pointer rounded bg-devil-interact/85 p-1.5 hover:bg-devil-interact"
            onClick={() => adjustMonth(1)}
          >
...
function DateRangeDayCell({
  ...
  const getBackgroundColor = useCallback(() => {
    switch (mode) {
      ...
      case DateRangeCellMode.SELECTED_START_NODE:
        return "!bg-devil-contrast rounded-tl-[5px] rounded-bl-[5px]";
      case DateRangeCellMode.SELECTED_END_NODE:
        return "!bg-devil-contrast rounded-tr-[5px] rounded-br-[5px]";
      case DateRangeCellMode.SELECTED:
        return "bg-devil-interact";
      case DateRangeCellMode.SELECTED_OUT_OF_RANGE:
        return "bg-devil-fill";
    }
  }, [mode]);
  ...
  const getTextColor = useCallback(() => {
    switch (mode) {
      case DateRangeCellMode.OUT_OF_RANGE:
      case DateRangeCellMode.SELECTED_OUT_OF_RANGE:
        return "!text-devil-subtle";
      case DateRangeCellMode.SELECTED_START_NODE:
      case DateRangeCellMode.SELECTED_END_NODE:
        return "!text-devil-inverse";
      default:
        return "text-devil-default";
    }
  }, [mode]);
  ...
  return (
    <button
      ...
      className={cn(
        ...
        sizeConfig.textSize,
        "cursor-pointer text-center text-devil-default transition-all duration-[50]",
    ...
    <div>
      <div className="mb-3 text-center">
        <input
          ...
          className={cn(
            "w-full rounded-md border-none bg-transparent py-1.5 text-center font-semibold text-devil-default transition-all duration-200 focus:ring-[1.5px] focus:ring-devil-focus/50 focus:outline-none",
            sizeConfig.textSize,
          )}
          onBlur={(e) => {
      ...
      <div className="mt-2 grid grid-cols-7 gap-1">
        {DAYS_OF_WEEK.map((day) => (
          <div
            key={day}
            className={cn(
              "h-[22px] text-center text-devil-subtle",
              sizeConfig.cellWidth,
              sizeConfig.textSize,
            )}
          >
...
function DateRangeFooter({
  ...
  return (
    <div
      className={cn(
        "flex items-center gap-2 text-devil-subtle",
        sizeConfig.textSize,
      )}
    >
      ...
      <button
        type="button"
        onClick={reset}
        className="cursor-pointer font-semibold text-devil-default underline underline-offset-2"
      >
        Reset Dates
      </button>
    </div>
