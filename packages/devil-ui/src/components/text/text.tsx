...
export const DEVIL_TEXT_VARIANTS = {
  variant: {
    heading: {
    ...
    body: {
      classes: "text-devil-default",
      description: "Default body text",
    },
    secondary: {
      classes: "text-devil-subtle",
      description: "Muted text for secondary information",
    },
    success: {
      classes: "text-devil-link",
      description: "Success state text",
    },
    error: {
      classes: "text-devil-danger",
    ...
    "mono-secondary": {
      classes: "font-mono text-devil-subtle",
...
export const DEVIL_TEXT_DEFAULT_VARIANTS = {
  variant: "body",
  size: "base",
} as const;
...
export const DEVIL_TEXT_STYLING = {
  ...
  baseColor: "text-devil-default",
  variantColors: {
    body: "text-devil-default",
    secondary: "text-devil-subtle",
    success: "text-devil-link",
    error: "text-devil-danger",
    mono: "text-devil-default",
    "mono-secondary": "text-devil-subtle",
  },
  fontFamilies: {
    default: "sans-serif",
    mono: "monospace",
...
// Derived types from DEVIL_TEXT_VARIANTS
export type DevilTextVariant = keyof typeof DEVIL_TEXT_VARIANTS.variant;
export type DevilTextSize = keyof typeof DEVIL_TEXT_VARIANTS.size;
...
type TextSize = DevilTextSize;
type TextVariant = DevilTextVariant;
...
function resolveTextSizeClasses(variant: TextVariant, size: TextSize) {
  ...
  if (["mono", "mono-secondary"].includes(variant)) {
    ...
    return size === "lg"
      ? DEVIL_TEXT_VARIANTS.size.base.classes
      : DEVIL_TEXT_VARIANTS.size.sm.classes;
  }

  return resolveVariant(
    DEVIL_TEXT_VARIANTS.size,
    size,
    DEVIL_TEXT_DEFAULT_VARIANTS.size,
  ).classes;
}

export interface DevilTextVariantsProps {
  variant?: DevilTextVariant;
  size?: DevilTextSize;
}

export function textVariants({
  variant = DEVIL_TEXT_DEFAULT_VARIANTS.variant,
  size = DEVIL_TEXT_DEFAULT_VARIANTS.size,
}: DevilTextVariantsProps = {}) {
  return cn(
    resolveVariant(
      DEVIL_TEXT_VARIANTS.variant,
      variant,
      DEVIL_TEXT_DEFAULT_VARIANTS.variant,
    ).classes,
...
function _Text<Variant extends TextVariant = "body">(
  ...
  if (process.env.NODE_ENV !== "production" && isDeprecatedHeading) {
    console.warn(
      `[Devil Text]: variant="${variant}" is deprecated. Use variant="heading" and set size and as explicitly.`,
    );
  }
