...
export const DEVIL_PAGINATION_VARIANTS = {
  controls: {
    full: {
      classes: "",
      description:
...
export type DevilPaginationControls =
  keyof typeof DEVIL_PAGINATION_VARIANTS.controls;

export const DEVIL_PAGINATION_DEFAULT_VARIANTS = {
  controls: "full",
} as const;

export interface DevilPaginationVariantsProps {
  controls?: DevilPaginationControls;
}

export function paginationVariants({
  controls = DEVIL_PAGINATION_DEFAULT_VARIANTS.controls,
}: DevilPaginationVariantsProps = {}) {
  return cn(
    "flex items-center justify-between gap-2",
    resolveVariant(
      DEVIL_PAGINATION_VARIANTS.controls,
      controls,
      DEVIL_PAGINATION_DEFAULT_VARIANTS.controls,
    ).classes,
  );
}
...
function PaginationInfo({ children, className }: PaginationInfoProps) {
  ...
  return (
    <div
      ...
      className={cn("text-sm text-devil-subtle", className)}
    >
      {content}
    </div>
  );
...
function PaginationPageSize({
  ...
  return (
    ...
    >
      {label && <span className="text-sm text-devil-subtle">{label}</span>}
      <Select
...
export interface PaginationControlsProps extends DevilPaginationVariantsProps {
  /**
          ...
          {showFullControls &&
            (pageSelector === "dropdown" ? (
              <Select
                ...
                className="rounded-none ring-devil-hairline"
                value={page}
                onValueChange={(value) => {
...
function PaginationSeparator({ className }: PaginationSeparatorProps) {
  return (
    <div
      ...
      className={cn("mx-2 h-6 border-l border-devil-hairline", className)}
    />
  );
}
...
export interface PaginationLegacyProps
  extends PaginationBaseProps, DevilPaginationVariantsProps {
  children?: never;
...
function PaginationRoot(props: PaginationProps) {
  ...
  const controls =
    "controls" in props
      ? (props.controls ?? DEVIL_PAGINATION_DEFAULT_VARIANTS.controls)
      : DEVIL_PAGINATION_DEFAULT_VARIANTS.controls;
    ...
    <PaginationContext.Provider value={contextValue}>
      ...
      >
        <div
          ...
          aria-atomic="true"
          data-slot="pagination-info"
          className="grow text-sm text-devil-subtle"
        >
          {getPaginationText()}
        </div>
