...
export const DEVIL_SWITCH_VARIANTS = {
  size: {
    sm: {
      classes: "h-5.5 w-8.5",
...
export const DEVIL_SWITCH_DEFAULT_VARIANTS = {
  size: "base",
  variant: "default",
} as const;

// Derived types from DEVIL_SWITCH_VARIANTS
export type DevilSwitchSize = keyof typeof DEVIL_SWITCH_VARIANTS.size;
export type DevilSwitchVariant = keyof typeof DEVIL_SWITCH_VARIANTS.variant;

export interface DevilSwitchVariantsProps {
  /**
   * Switch size.
   ...
   * @default "default"
   */
  variant?: DevilSwitchVariant;
}

export function switchVariants({
  size = DEVIL_SWITCH_DEFAULT_VARIANTS.size,
  variant = DEVIL_SWITCH_DEFAULT_VARIANTS.variant,
}: DevilSwitchVariantsProps = {}) {
  const sizeConfig = resolveVariant(
    DEVIL_SWITCH_VARIANTS.size,
    size,
    DEVIL_SWITCH_DEFAULT_VARIANTS.size,
  );
  const variantConfig = resolveVariant(
    DEVIL_SWITCH_VARIANTS.variant,
    variant,
    DEVIL_SWITCH_DEFAULT_VARIANTS.variant,
  );
  return cn(sizeConfig.classes, variantConfig.classes);
}

// Legacy type aliases for backwards compatibility
export type SwitchSize = DevilSwitchSize;
export type SwitchVariant = DevilSwitchVariant;
      ...
      <BaseSwitch.Root
        ...
        render={(rootProps, state) => {
          ...
          const trackColors = isNeutral
            ? state.checked
              ? "bg-neutral-500 dark:bg-devil-base ring-neutral-600 dark:ring-neutral-700"
              : "bg-neutral-150 dark:bg-devil-base ring-devil-hairline"
            : state.checked
              ? "bg-devil-brand ring-devil-brand"
          ...
          const thumbColors = isNeutral
            ? state.checked
              ? "bg-devil-base dark:bg-neutral-400"
              : "bg-devil-base dark:bg-neutral-850"
            : state.checked
              ? "bg-devil-base"
              : "bg-devil-base dark:bg-neutral-850";
          ...
          const trackClassName = cn(
            ...
            "focus:outline-none focus-visible:ring-2 focus-visible:ring-devil-brand",
          ...
          const thumbClassName = cn(
            "absolute top-0 bottom-0 shadow-[0_0_1px_0.5px_var(--color-devil-shadow-edge),0_1px_2px_var(--color-devil-shadow-drop)]",
            s.thumb,
            squircleRadius,
            thumbColors,
  ...
  (
    ...
    return (
      ...
      >
        ...
        />
        <span className="text-base font-medium text-devil-default">{label}</span>
      </label>
    );
  },
);
