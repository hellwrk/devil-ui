...
export const DEVIL_DIALOG_VARIANTS = {
  size: {
    base: {
      classes: "sm:w-96",
...
export const DEVIL_DIALOG_DEFAULT_VARIANTS = {
  size: "base",
  role: "dialog",
} as const;

export const DEVIL_DIALOG_STYLING = {
  dimensions: {
    sm: {
      width: 350,
      titleSize: 20,
...
// Derived types from DEVIL_DIALOG_VARIANTS
export type DevilDialogSize = keyof typeof DEVIL_DIALOG_VARIANTS.size;
export type DevilDialogRole = keyof typeof DEVIL_DIALOG_VARIANTS.role;

export interface DevilDialogVariantsProps {
  /**
   * Dialog width.
...
const DialogRoleContext = createContext<DevilDialogRole>("dialog");
...
export function dialogVariants({
  size = DEVIL_DIALOG_DEFAULT_VARIANTS.size,
}: DevilDialogVariantsProps = {}) {
  return cn(
    // Base styles
    "shadow-m ring ring-devil-line fixed top-8 left-1/2 sm:top-16 w-full max-w-[calc(100vw-2rem)] -translate-x-1/2 overflow-hidden rounded-xl bg-devil-base text-devil-default duration-150 data-ending-style:scale-90 data-ending-style:opacity-0 data-starting-style:scale-90 data-starting-style:opacity-0",
    // Apply size from DEVIL_DIALOG_VARIANTS
    resolveVariant(
      DEVIL_DIALOG_VARIANTS.size,
      size,
      DEVIL_DIALOG_DEFAULT_VARIANTS.size,
    ).classes,
  );
}
 ...
 * ```
 */
export type DialogProps = DevilDialogVariantsProps & {
...
function DialogContent({
  ...
  return (
    <BasePortal container={container}>
      <BaseBackdrop className="fixed inset-0 bg-devil-recessed opacity-80 transition-all duration-150 data-ending-style:opacity-0 data-starting-style:opacity-0" />
      <LayerCard
        render={<BasePopup />}
...
function DialogRoot(props: DialogRootProps) {
  ...
  const {
    children,
    role = DEVIL_DIALOG_DEFAULT_VARIANTS.role,
    ...rootProps
  } = props;
...
function DialogClose({ children, ...props }: DialogCloseProps) {
  ...
  return (
    <BaseClose data-devil-component="Dialog" data-devil-part="close" {...props}>
      {children}
    </BaseClose>
  );
}
