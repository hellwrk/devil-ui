...
export const DEVIL_MENUBAR_VARIANTS = {
  // MenuBar currently has no variant options but structure is ready for future additions
} as const;

export const DEVIL_MENUBAR_DEFAULT_VARIANTS = {} as const;

// Derived types from DEVIL_MENUBAR_VARIANTS
export interface DevilMenuBarVariantsProps {}

export function menuBarVariants(_props: DevilMenuBarVariantsProps = {}) {
  return cn(
    // Base styles
    "flex rounded-lg border border-devil-recessed bg-devil-recessed pl-px shadow-xs transition-colors",
  );
}
...
const MenuOption = ({
  ...
  const button = (
    <button
      data-devil-component="MenuBar"
      data-devil-part="option"
      aria-label={tooltip}
      className={cn(
        "relative -ml-px flex h-full w-11 cursor-pointer items-center justify-center rounded-md border-none bg-devil-recessed transition-colors first:rounded-l-lg last:rounded-r-lg focus:z-3 focus:ring-devil-focus/50 focus:outline-none focus-visible:z-3 focus-visible:ring-2 focus-visible:ring-devil-brand",
        {
          "z-2 bg-devil-base shadow-xs transition-colors": isActive === id,
        },
      )}
      onClick={onClick}
    >
...
export const MenuBar = ({
  ...
  return (
    <nav
      className={cn(
        "isolate flex rounded-lg bg-devil-recessed pl-px shadow-xs ring ring-devil-line transition-colors",
        className,
      )}
      ref={menuRef}
    >
