...
import { DevilPortalProvider } from "../../utils/portal-provider";
import { DevilLocaleProvider } from "../../utils/locale-provider";
import {
  DEVIL_LAYER_DIALOG_DEFAULT_VARIANTS,
  DEVIL_LAYER_DIALOG_VARIANTS,
  LayerDialog,
} from "./layer-dialog";
...
describe("LayerDialog", () => {
  ...
  it("uses a larger constrained desktop width by default", () => {
    expect(DEVIL_LAYER_DIALOG_DEFAULT_VARIANTS.size).toBe("base");
    expect(DEVIL_LAYER_DIALOG_VARIANTS.size.base.classes).toBe("sm:max-w-xl");
    expect(Object.keys(DEVIL_LAYER_DIALOG_VARIANTS.size)).toEqual([
      "sm",
      "base",
      "lg",
      "xl",
  ...
  it("falls back to default variants for unknown size and alignment", () => {
    ...
    expect(getByRole("dialog").className).toContain(
      DEVIL_LAYER_DIALOG_VARIANTS.size.base.classes,
    );
  ...
  it("uses the portal container from DevilPortalProvider", () => {
    const portalContainer = document.createElement("div");
    document.body.append(portalContainer);

    const { unmount } = render(
      <DevilPortalProvider container={portalContainer}>
        <LayerDialog.Root open>
          <LayerDialog.Content>
...
describe("LayerDialog dismissal", () => {
  ...
  it("keeps the primary action neutral unless destructive is requested", () => {
    ...
    const emphasisToken = (button: HTMLElement) =>
      button.style.getPropertyValue("--devil-button-emphasis-gradient-end");
