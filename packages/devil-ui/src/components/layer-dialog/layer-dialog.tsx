...
import { useDevilLocale } from "../../utils/locale-provider";

export const DEVIL_LAYER_DIALOG_VARIANTS = {
  size: {
    sm: {
      classes: "sm:max-w-md",
...
export const DEVIL_LAYER_DIALOG_DEFAULT_VARIANTS = {
  size: "base",
  verticalAlign: "center",
} as const;

export type DevilLayerDialogSize = keyof typeof DEVIL_LAYER_DIALOG_VARIANTS.size;
export type DevilLayerDialogVerticalAlign =
  keyof typeof DEVIL_LAYER_DIALOG_VARIANTS.verticalAlign;
...
export interface LayerDialogContentProps {
  children: ReactNode;
  /**
   * Container element for the portal. Overrides `DevilPortalProvider` context.
...
function LayerDialogContent({
  ...
  size = DEVIL_LAYER_DIALOG_DEFAULT_VARIANTS.size,
  verticalAlign = DEVIL_LAYER_DIALOG_DEFAULT_VARIANTS.verticalAlign,
}: LayerDialogContentProps) {
  const { layerDialog } = useDevilLocale();
  ...
  const sizeConfig = resolveVariant(
    DEVIL_LAYER_DIALOG_VARIANTS.size,
    size,
    DEVIL_LAYER_DIALOG_DEFAULT_VARIANTS.size,
  );
  const verticalAlignConfig = resolveVariant(
    DEVIL_LAYER_DIALOG_VARIANTS.verticalAlign,
    verticalAlign,
    DEVIL_LAYER_DIALOG_DEFAULT_VARIANTS.verticalAlign,
  );
    ...
    <DrawerBase.Portal container={container}>
      ...
      */}
      ...
      >
        ...
        >
          <LayerCard className="flex max-h-[85dvh] min-h-0 w-full flex-col overflow-hidden rounded-none bg-devil-elevated p-1.5 shadow-[0_20px_25px_-5px_rgb(0_0_0/0.03),0_8px_10px_-6px_rgb(0_0_0/0.03)] max-sm:border-t max-sm:border-devil-hairline max-sm:shadow-xs max-sm:ring-0 sm:max-h-full sm:rounded-xl">
            {!isDesktop && !isAlert && (
              <div aria-hidden className="flex justify-center pt-1.5 pb-3">
                <div className="h-1 w-10 rounded-full bg-devil-fill" />
              </div>
            )}
...
function LayerDialogBody({ children }: LayerDialogBodyProps) {
  ...
  return (
    <LayerCard.Primary className="min-h-0 flex-1 gap-0 p-0">
      <div className="z-10 flex shrink-0 items-start justify-between gap-4 rounded-t-lg bg-devil-base px-4.5 py-4">
      ...
      <ScrollAreaBase.Root className="relative flex min-h-0 flex-1 flex-col">
        ...
        >
          <ScrollAreaBase.Thumb className="w-full rounded-full bg-devil-contrast opacity-10 transition-opacity hover:opacity-20 active:opacity-30" />
        </ScrollAreaBase.Scrollbar>
      </ScrollAreaBase.Root>
    </LayerCard.Primary>
  );
...
> & {
   ...
   * @default "primary"
   */
  variant?: DevilLayerDialogPrimaryVariant;
};

export type DevilLayerDialogPrimaryVariant = "primary" | "destructive";
...
function LayerDialogDismiss({
  ...
  const close = (closeProps: ComponentPropsWithoutRef<"button">) => (
    <Button
      {...closeProps}
      className="hover:bg-devil-fill/50"
      disabled={disabled}
      variant="ghost"
    >
      {label}
