...
describe("Select", () => {
  ...
  describe("surface styling", () => {
    ...
    it("does not revert to the Button surface when open or disabled", () => {
      ...
      expect(trigger.className).toContain("data-[state=open]:bg-devil-control");
      expect(trigger.className).toContain("disabled:bg-devil-control/50");
      expect(trigger.className).not.toContain("data-[state=open]:bg-devil-base");
      expect(trigger.className).not.toContain("disabled:bg-devil-base/50");
    });

    it("allows className to override the default surface without important", () => {
      render(
        <Select aria-label="Pick one" className="bg-devil-elevated">
          <Select.Option value="a">Option A</Select.Option>
        </Select>,
      );
      ...
      expect(classes).toContain("bg-devil-elevated");
      expect(classes).not.toContain("bg-devil-control");
    });
  });
  ...
  describe("groups and separators", () => {
    ...
    it("renders Select.Separator as a visual divider", async () => {
      ...
      expect(separatorRef.current?.className).toContain("bg-devil-hairline");
    });
  ...
  describe("popup structure", () => {
    ...
    it("uses a presentation popup and inner listbox after the refactor", async () => {
      ...
      expect(popup?.className).toContain("bg-devil-base");
      expect(popup?.className).not.toContain("bg-devil-control");
