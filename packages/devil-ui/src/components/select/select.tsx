...
import { DEVIL_INPUT_VARIANTS, type DevilInputSize } from "../input/input";
...
export const DEVIL_SELECT_VARIANTS = {
  size: DEVIL_INPUT_VARIANTS.size,
} as const;

export const DEVIL_SELECT_DEFAULT_VARIANTS = {
  size: "base",
} as const;
...
// Derived types from DEVIL_SELECT_VARIANTS
export type DevilSelectSize = keyof typeof DEVIL_SELECT_VARIANTS.size;

export interface DevilSelectVariantsProps {
  /**
...
export function selectVariants({
  size = DEVIL_SELECT_DEFAULT_VARIANTS.size,
}: DevilSelectVariantsProps = {}) {
  return cn(
    buttonVariants({ size }),
    "bg-devil-control disabled:bg-devil-control/50 data-[state=open]:bg-devil-control",
    "justify-between font-normal",
    "focus:opacity-100 focus:ring-devil-focus/50 focus-visible:ring-inset *:in-focus:opacity-100",
  );
}
...
> = {
  xs: { iconSize: 12, className: "text-devil-subtle" },
  sm: { iconSize: 14, className: "text-devil-subtle" },
  base: { iconSize: 16, className: "text-devil-subtle" },
  lg: { iconSize: 18, className: "text-devil-subtle" },
};
 ...
 * Wraps Base UI Select with Devil styling and optional Field integration.
 *
 * @example
 * ```tsx
...
export function Select<T, Multiple extends boolean | undefined = false>({
  ...
  if (process.env.NODE_ENV !== "production" && hideLabel !== undefined) {
    console.warn(
      "[Devil Select]: `hideLabel` is deprecated. For hidden labels, use `aria-label` instead of `label` + `hideLabel={true}`.\n" +
        "  Migration:\n" +
  ...
  const valueChildrenFn = renderValue
    ? (value: unknown) => {
        const placeholderNode =
          placeholder != null ? (
            <span className="text-devil-placeholder">{placeholder}</span>
          ) : null;
  ...
  // Exclude Devil-extended `items` from Base UI spread — we pass `normalizedItems` instead
  ...
  const selectLabelNode = label ? (
    <SelectBase.Label className="m-0 text-base font-medium text-devil-default select-none">
      <Label
  ...
  const selectTrigger = (
    <SelectBase.Trigger
      ...
      className={cn(
        ...
        error &&
          "!ring-devil-danger focus:ring-[1.5px] focus:ring-devil-danger/50",
        className,
      )}
    ...
    >
      {loading ? (
        ...
        <SelectBase.Value
          ...
          className="min-w-0 truncate data-[placeholder]:text-devil-placeholder"
        >
          {valueChildrenFn}
        </SelectBase.Value>
      )}
        ...
        */}
        ...
        >
          <SelectBase.Popup
            className={cn(
              "flex flex-col",
              "max-h-[var(--available-height)] bg-devil-base text-devil-default",
              "rounded-lg shadow-lg ring ring-devil-line",
  ...
  return (
    <div className="grid gap-2">
      ...
      {selectControl}
      {normalizedError ? (
        <span className="text-sm text-devil-danger">
          {normalizedError.message}
        </span>
      ) : (
        description && (
          <span className="text-sm leading-snug text-devil-subtle">
            {description}
          </span>
        )
      )}
...
function Option<T>({ children, value, disabled, className }: OptionProps<T>) {
  return (
    <SelectBase.Item
      data-devil-component="Select"
      data-devil-part="option"
      value={value}
      disabled={disabled}
      className={cn(
        "group mx-1.5 flex cursor-pointer items-center justify-between gap-2 rounded px-2 py-1.5 text-base outline-none",
        "focus-visible:z-50 focus-visible:ring-2 focus-visible:ring-devil-brand focus-visible:ring-inset",
        "data-highlighted:bg-devil-tint",
...
const GroupLabel = forwardRef<HTMLDivElement, GroupLabelProps>(
  ({ children, className }, ref) => (
    <SelectBase.GroupLabel
      ref={ref}
      className={cn(
        "px-3.5 py-1.5 text-sm font-semibold text-devil-subtle",
        className,
      )}
    >
      {children}
