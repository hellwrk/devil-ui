...
export function CodeHighlighted({
  ...
  const handleCopy = useCallback(async () => {
    try {
      ...
      console.error("[Devil CodeHighlighted] Failed to copy to clipboard:", err);
    }
  }, [code]);
  ...
  const containerClasses = cn(
    "group relative m-0 w-full min-w-0 rounded-md border border-devil-fill bg-devil-base p-0",
  ...
  const lineNumbers =
    showLineNumbers && !isSingleLine ? (
      <div
        className="devil-line-numbers shrink-0 py-4 pr-4 text-right font-mono text-sm opacity-40 select-none"
        aria-hidden="true"
      >
  ...
  useEffect(() => {
    if (error) {
      console.error(
        "[Devil CodeHighlighted] Shiki initialization error:",
        error,
      );
    }
  }, [error]);
    ...
    return (
      <div className={containerClasses}>
        {lineNumbers && (
          <div className="flex">
            {lineNumbers}
            <pre className="!m-0 min-w-0 flex-1 overflow-x-auto !p-4 font-mono text-sm leading-relaxed text-devil-subtle">
      ...
      {lineNumbers && (
        <div className="flex w-full">
          ...
          <div className="min-w-0 flex-1 overflow-x-auto">
            <div
              className="devil-shiki [&_code]:!m-0 [&_code]:!border-0 [&_code]:!bg-transparent [&_code]:!p-0 [&>pre]:!m-0 [&>pre]:!rounded-none [&>pre]:!border-0 [&>pre]:!bg-transparent [&>pre]:!p-4 [&>pre]:font-mono [&>pre]:text-sm [&>pre]:leading-relaxed"
