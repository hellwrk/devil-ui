 ...
 * @example
 * ```tsx
 * import { useShikiHighlighter } from "@hellwrk/devil-ui/code";
 *
  ...
  if (!context) {
    throw new Error(
      "useShikiHighlighter must be used within a ShikiProvider. " +
        "Wrap your app with <ShikiProvider> from '@hellwrk/devil-ui/code'.",
    );
  }
  ...
  const highlight = useCallback(
    (code: string, lang: string): string | null => {
      ...
      if (!normalizedLang || !languages.includes(normalizedLang)) {
        console.warn(
          `[Devil CodeHighlighted] Language "${lang}" is not in the ShikiProvider's languages list. ` +
      ...
      try {
        ...
        console.warn(
          `[Devil CodeHighlighted] Failed to highlight code with language "${lang}":`,
          err,
        );
        return null;
      }
