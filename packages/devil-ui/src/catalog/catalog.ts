/**
 * Devil Catalog - Runtime validation and AI prompt generation.
 *
...
interface SchemasModule {
  DEVIL_COMPONENT_NAMES: readonly string[];
  UIElementBaseSchema: {
...
/**
 * Create a Devil catalog for runtime validation.
 *
 * The catalog:
 ...
 *
 * @example
 * import { createDevilCatalog, initCatalog } from '@hellwrk/devil-ui/catalog';
 *
 ...
 * }
 */
export function createDevilCatalog(config: CatalogConfig = {}): DevilCatalog {
  ...
  return {
    get componentNames(): readonly string[] {
      ...
      return schemas.DEVIL_COMPONENT_NAMES;
    },
    ...
    hasComponent(type: string): boolean {
      try {
        const schemas = getSchemas();
        return schemas.DEVIL_COMPONENT_NAMES.includes(type as never);
      } catch {
        return false;
      }
    },
    ...
    generatePrompt(): string {
      const schemas = getSchemas();
      const lines: string[] = [
        "# Devil Component Catalog",
        "",
        "You are generating UI using Devil components. Output must be valid JSON matching the UITree schema.",
        "",
      ...
      // List all components
      for (const name of schemas.DEVIL_COMPONENT_NAMES) {
        lines.push(`- \`${name}\``);
      }
 ...
 */
export async function initCatalog(_catalog?: DevilCatalog): Promise<void> {
  await loadSchemas();
}
