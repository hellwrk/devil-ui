 ...
 *
 * @param variants  – A dimension of a DEVIL_*_VARIANTS object
 *                    (e.g. `DEVIL_BUTTON_VARIANTS.variant`)
 * @param key       – The runtime value of the variant prop
 * @param fallback  – A key guaranteed to exist (typically from
 *                    DEVIL_*_DEFAULT_VARIANTS)
...
export function resolveVariant<T extends Record<string, unknown>>(
  ...
  if (process.env.NODE_ENV !== "production") {
    console.warn(
      `[devil] Unknown variant "${key}". Expected one of: ${Object.keys(variants).join(", ")}. Falling back to "${fallback}".`,
    );
  }
