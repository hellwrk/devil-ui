...
export const DEVIL_DELETE_RESOURCE_VARIANTS = {
  size: {
    sm: {
      classes: "",
...
export const DEVIL_DELETE_RESOURCE_DEFAULT_VARIANTS = {
  size: "base",
} as const;

export type DevilDeleteResourceSize =
  keyof typeof DEVIL_DELETE_RESOURCE_VARIANTS.size;

export interface DevilDeleteResourceVariantsProps {
  size?: DevilDeleteResourceSize;
}

export interface DeleteResourceProps extends DevilDeleteResourceVariantsProps {
...
export function DeleteResource({
  ...
  caseSensitive = true,
  deleteButtonText,
  size = DEVIL_DELETE_RESOURCE_DEFAULT_VARIANTS.size,
  errorMessage,
  className,
}: DeleteResourceProps) {
  ...
  return (
    <DialogRoot open={open} onOpenChange={onOpenChange}>
      <Dialog size={size} className={cn("p-0", className)}>
        <div className="flex items-center justify-between border-b border-devil-line px-6 py-4">
        ...
        <div className="flex flex-col gap-4 p-6">
          <div className="flex flex-col gap-2">
            ...
            <p className="max-w-prose text-base text-pretty text-devil-subtle">
              This action cannot be undone. This will permanently delete the{" "}
              <span className="font-medium text-devil-default">
                {resourceName}
              </span>{" "}
          ...
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-1.5 text-base">
              <span>
                Type{" "}
                <button
                  className="group inline rounded-md bg-devil-tint px-2 py-1 font-mono text-sm font-semibold hover:cursor-pointer hover:bg-devil-fill"
                  onClick={handleCopy}
                ...
                >
                  ...
                  {copied ? (
                    ...
                    <CopyIcon
                      size={12}
                      weight="bold"
                      className="ml-1.5 inline text-devil-subtle group-hover:text-devil-default"
                    />
                  )}
                </button>{" "}
                to confirm:
        ...
        <div className="flex justify-end gap-3 border-t border-devil-line px-6 py-4">
          <DialogClose
            render={(props) => (
