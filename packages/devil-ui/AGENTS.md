...
│   ├── blocks/              # Installable blocks (NOT library exports; via CLI `devil add`)
...
│   ├── styles/              # CSS: devil-binding.css + theme files (AUTO-GENERATED)
...
| Variant definitions      | `DEVIL_{NAME}_VARIANTS` export in component file | Machine-readable + lint-enforced                                           |
...
- **Path aliases**: `@/` → `src/`, `devil-ui` → `src/index.ts`
...
- **5th lint rule** (`no-deprecated-props`): Only in `packages/devil-ui/lint/`, reads deprecation data from registry
