...
describe("Primitives Export", () => {
  ...
  describe("Sync with base-ui", () => {
    ...
    it("should export all base-ui primitives from devil", async () => {
      ...
      // Import devil primitives
      const devilPrimitives = await import("../../src/primitives/index.ts");
      const devilExportNames = Object.keys(devilPrimitives);

      // For each base-ui export, check that devil re-exports something from it
      ...
      for (const exportName of baseUiExports) {
        ...
        if (!devilExportNames.includes(pascalName)) {
      ...
      if (missingExports.length > 0) {
        console.error(
          "\n❌ Base-ui primitives missing from devil-ui/primitives:",
        );
