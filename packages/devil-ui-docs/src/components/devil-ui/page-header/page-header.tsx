...
import type { TabsItem } from "@hellwrk/devil-ui";
...
export function PageHeader({
  ...
  return (
    <div className={cn(pageHeaderVariants({ spacing }), className)}>
      <div className="border-b border-devil-hairline">{breadcrumbs}</div>

      {(title || description) && (
        <div className="flex flex-col gap-2 py-3 pl-3">
          {title && (
            <h1 className="font-heading text-3xl font-semibold tracking-tight text-devil-default">
              {title}
            </h1>
          )}
          {description && (
      ...
      {tabs && (
        <div className="flex w-full items-center justify-between border-b border-devil-hairline pt-1 pb-3 pl-3">
          <Tabs
            tabs={tabs}
