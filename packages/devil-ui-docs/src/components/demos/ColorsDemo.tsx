...
import { devilColors, type ColorToken } from "virtual:kumo-colors";
...
const ColorSwatch: FC<{ label: string; value: string }> = ({
  ...
  return (
    <div className="flex min-w-0 items-start gap-2">
      <span
        className="inline-flex h-8 w-8 shrink-0 rounded border border-devil-fill"
        style={{ background: value }}
      />
      <div className="flex min-w-0 flex-col text-xs text-devil-default">
        ...
        <span className="text-[10px] leading-tight break-normal opacity-60">
          {value}
          {hex && (
            <span className="ml-1 font-mono font-medium text-devil-default">
              {hex}
            </span>
          )}
        </span>
...
function getTheme(): string {
  return document.body.getAttribute("data-theme") ?? "devil";
}

function useCurrentTheme(): string {
  return useSyncExternalStore(subscribeToTheme, getTheme, () => "devil");
}
...
const componentNames: Set<string> = new Set(
  Object.keys(devilRegistryJson.components).map((n) => n.toLowerCase()),
);

/**
 * Extract the component name from a token, if it matches a known component.
 *
 * Token format: `--color-devil-{component}-{variant}` or `--text-color-devil-{component}-{variant}`
...
function getComponentFromToken(tokenName: string): string | null {
  ...
  // "--color-devil-badge-red" → "devil-badge-red"
  // "--text-color-devil-badge-red-subtle" → "devil-badge-red-subtle"
  const semantic = tokenName
    .replace(/^--text-color-/, "")
    .replace(/^--color-/, "");

  // Must start with "devil-"
  if (!semantic.startsWith("devil-")) return null;

  // "devil-badge-red" → "badge-red" → check if "badge" is a component
  const afterDevil = semantic.slice("devil-".length);
  const segments = afterDevil.split("-");

  // Require at least 2 segments: component name + variant.
  // This avoids matching standalone semantic tokens like "devil-link" or "devil-surface".
...
function getColorsForTheme(theme: string): ColorsByCategory {
  ...
  const componentGroups: ComponentColorGroup[] = [
    ...
    .map(([comp, tokens]) => {
      ...
      const registryName = Object.keys(devilRegistryJson.components).find(
...
const TokenGrid: FC<{ tokens: ColorToken[] }> = ({ tokens }) => (
  <div className="grid grid-cols-1 gap-3 md:grid-cols-2 lg:grid-cols-2">
    {tokens.map((token: ColorToken) => (
      <div
        key={token.name}
        className={`flex min-w-0 items-center gap-3 rounded-md border bg-devil-base px-3 py-2 text-xs ${
          token.tokenType === "global"
            ? "border border-devil-info ring-1 ring-devil-info/30"
            : "border-devil-fill"
        }`}
      >
        <div className="flex min-w-0 flex-1 flex-col gap-1">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {token.tokenType === "global" && (
              <span className="rounded bg-devil-info/20 px-1.5 py-0.5 text-[10px] font-medium text-devil-link">
                global
              </span>
            )}
          </div>
...
export const TailwindColorTokens: FC = () => {
  ...
  return (
    <div className="flex flex-col gap-6 text-devil-default">
        ...
        <div className="text-sm text-devil-default">
          ...
          {overrideCount > 0 && (
            <span className="ml-1">
              ...
              <code className="rounded bg-devil-brand p-1">{currentTheme}</code>
            </span>
          )}
        </div>
      </div>
        ...
        <section className="flex flex-col gap-3">
          ...
          {componentGroups.map((group) => (
            <div key={group.component} className="flex flex-col gap-3">
              <h3 className="!m-0 text-xs font-semibold text-devil-subtle">
...
export function StatusBannerDemo() {
  return (
    <div className="flex items-center gap-2 rounded-lg bg-devil-danger-tint/70 p-4">
      <WarningIcon weight="fill" className="fill-devil-danger" />
      <span className="text-sm text-devil-danger">Something went wrong.</span>
    </div>
  );
}
