...
const HELP_TEXT = `Devil CLI - Component registry and blocks distribution
...
function executeCommand(
  ...
  // Handle non-devil commands
  if (baseCommand !== "devil" && baseCommand !== "npx") {
    if (!trimmed) {
      return { output: [], isError: false };
    }
    return {
      output: [`command not found: ${baseCommand}`, 'Try "devil help"'],
      isError: true,
    };
  }
  ...
  let commandIndex = 1;
  if (baseCommand === "npx") {
    // Skip the package part ("@hellwrk/devil-ui" or "devil-ui")
    if (parts[1] === "@hellwrk/devil-ui" || parts[1] === "devil-ui") {
      commandIndex = 2;
    } else {
      return {
  ...
  switch (command) {
    case "ls": {
      ...
      const output: string[] = [];
      output.push(`Devil Components (${components.length} total)`);
      output.push("");
    ...
    case "docs": {
      if (!arg && command === "doc") {
        ...
        const output: string[] = [];
        output.push(`# Devil Component Documentation`);
        output.push("");
        output.push(
          `${Object.keys(registry.components).length} components available`,
        );
        output.push("");
        output.push('Use "devil doc <ComponentName>" for detailed docs');
        output.push("");
      ...
      if (arg) {
        ...
        if (!component) {
          ...
          output.push("");
          output.push('Run "devil ls" to see all available components.');
      ...
      output.push("");
      output.push('Use "devil doc <name>" to see a specific component.');
    ...
    case "init": {
      const output: string[] = [];
      output.push("Initializing Devil configuration...");
      output.push("");
      output.push(
        "Where should blocks be installed? (src/components/devil-ui): src/components/devil-ui",
      );
      output.push("");
      output.push("Created devil-ui.json with the following configuration:");
      output.push("   Blocks directory: src/components/devil-ui");
      output.push("");
      output.push("Next steps:");
      output.push('  1. Run "devil blocks" to see available blocks');
      output.push('  2. Run "devil add <block-name>" to install a block');
      return { output, isError: false };
    }
    ...
    case "blocks": {
      ...
      const output: string[] = [];
      output.push(`Devil Blocks (${blocks.length} total)`);
      output.push("");
      ...
      output.push("  devil add <block-name>");
      output.push("");
      output.push("Example:");
      output.push("  devil add PageHeader");
    ...
    case "add": {
      if (!arg) {
        return {
          output: [
            ...
            "",
            "Usage: devil add <block-name>",
            'Run "devil blocks" to see available blocks.',
          ],
          isError: true,
        };
      }
      ...
      if (!block) {
        ...
        if (matchedBlock) {
          return executeCommand(`devil add ${matchedBlock.name}`, registry);
        }

        const output = [`Error: Block '${blockName}' not found.`];
        output.push('Run "devil blocks" to see available blocks.');
        return { output, isError: true };
      }
      ...
      if (block.dependencies && block.dependencies.length > 0) {
        output.push("This block depends on the following Devil components:");
        for (const dep of block.dependencies) {
          output.push(`  - ${dep} (from devil-ui)`);
        }
        output.push("");
        output.push("Make sure devil-ui is installed in your project:");
        output.push("  pnpm add @hellwrk/devil-ui");
        output.push("");
      }
      ...
      if (mainFile) {
        ...
        output.push(
          `  import { ${block.name} } from "src/components/devil-ui/${importPath}";`,
        );
      }
...
export const CLITerminal: FC = () => {
  const [lines, setLines] = useState<TerminalLine[]>([
    {
      ...
      content:
        "Devil CLI v" +
        devilRegistryJson.version +
        ' - Type "devil help" for commands',
    },
  ]);
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="text-sm text-devil-default">
        <div className="flex flex-wrap gap-2">
          {[
            ...
            <button
              ...
              className="rounded bg-devil-overlay px-2 py-1 font-mono text-xs text-devil-default transition-colors hover:bg-devil-recessed"
            >
              {cmd}
            </button>
          ))}
      ...
      >
        ...
        {lines.map((line, i) => {
          ...
          return (
            ...
            >
              {line.type === "input" ? (
                <span className="text-devil-success">
                  <span>$</span> {line.content}
                </span>
              ) : line.type === "error" ? (
                <span className="text-devil-danger">{line.content}</span>
              ) : (
                <span className="text-devil-success opacity-90">
                  {line.content}
                </span>
              )}
            </div>
        ...
        <div className="flex items-center">
          <span className="text-devil-success">$</span>
          <span className="mx-1 text-devil-success">{currentInput}</span>
      ...
      <p className="text-xs text-devil-subtle">
