...
function DemoMain({ children }: { children?: React.ReactNode }) {
  return (
    <main className="flex flex-1 flex-col items-center justify-center gap-2 p-8 text-base text-devil-subtle">
...
function BrandLogo() {
  return (
    <div className="flex w-full min-w-0 items-center gap-2 px-3 transition-[padding] duration-(--sidebar-animation-duration) ease-(--sidebar-easing) group-data-[state=collapsed]/sidebar:px-2">
      <CubeIcon className="size-4 shrink-0 text-devil-brand" weight="duotone" />
      <span className="flex-1 truncate text-sm font-semibold text-devil-strong">
        Company
      </span>
    </div>
  );
    ...
    <DropdownMenu>
      <DropdownMenu.Trigger
        render={
          <button
            type="button"
            className="flex w-full min-w-0 cursor-pointer items-center gap-2 rounded-lg px-3 py-2 text-left text-sm font-medium text-devil-default transition-[padding] duration-(--sidebar-animation-duration) ease-(--sidebar-easing) outline-none hover:bg-devil-tint focus-visible:ring-1 focus-visible:ring-devil-line"
          >
            <active.icon
              className="size-4 shrink-0 text-devil-brand"
              weight="duotone"
            />
            ...
            <span className="w-4 shrink-0 overflow-hidden transition-[width] duration-(--sidebar-animation-duration) ease-(--sidebar-easing) group-data-[state=collapsed]/sidebar:w-0">
              <CaretUpDownIcon className="size-4 text-devil-subtle" />
            </span>
          </button>
        }
      />
      <DropdownMenu.Content className="w-(--anchor-width)">
        {accounts.map((account) => (
          ...
          >
            <account.icon className="size-4 text-devil-brand" weight="duotone" />
            {account.name}
...
function AccountAvatar({ name, size = 28 }: { name: string; size?: number }) {
  return (
    <span
      ...
      className="flex shrink-0 items-center justify-center rounded-[30%] bg-devil-control text-devil-default shadow-xs ring ring-devil-line"
    >
      <span
      ...
      <DropdownMenu.Trigger
        render={
          <button
            ...
            className="shrink-0 cursor-pointer rounded-full transition-opacity outline-none hover:opacity-80 focus-visible:ring-2 focus-visible:ring-devil-line"
          >
...
function PeekStateIndicator() {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <span className="text font-medium text-devil-default">
        State: {labels[state]}
      </span>
...
function NavBadge({ children }: { children: React.ReactNode }) {
  return (
    <span className="ml-1.5 shrink-0 rounded-full border border-dashed border-devil-line px-1.5 py-px text-[10px] font-medium text-devil-subtle">
      {children}
    </span>
  );
}
...
function QuickSearch() {
  return (
    <button
      type="button"
      className="group flex h-8 w-full shrink-0 cursor-pointer items-center gap-2 overflow-x-clip rounded-lg border-0 bg-devil-base px-3 text-left text-sm font-normal !text-devil-default shadow-xs ring ring-devil-line transition-[color,background,border,box-shadow] duration-250 select-none not-disabled:hover:bg-devil-tint focus:ring-devil-focus/50 focus:outline-none focus-visible:ring-2 focus-visible:ring-devil-brand"
    >
      <MagnifyingGlassIcon className="size-4 shrink-0" />
      <span className="flex-1 text-left">Quick search...</span>
      <kbd className="shrink-0 font-sans text-xs text-devil-subtle">⌘K</kbd>
    </button>
  );
}
