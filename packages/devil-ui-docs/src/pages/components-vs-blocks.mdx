...
description: "Understanding how devil-ui delivers different types of building blocks: versioned components you import, and copy-paste blocks you own."
---
...
Not everything belongs in a design system library. devil-ui distinguishes between **components** (shared, versioned primitives) and **blocks** (patterns you copy and own). Understanding this distinction helps you build applications faster while maintaining the right level of ownership.
...
```tsx
import { Button, Input, Dialog, Badge } from "@hellwrk/devil-ui";

// Or use granular imports for better tree-shaking
import { Button } from "@hellwrk/devil-ui/components/button";
```

When devil-ui ships a new version, you get bug fixes and improvements automatically. Your code doesn't change — just bump the version.
...
- **Composition-first** — Built from devil-ui components, not raw HTML
...
```tsx
// Path depends on your devil-ui.json blocksDir setting
// Default: src/components/devil-ui/
import { PageHeader } from "./components/devil-ui/page-header/page-header";
```
...
<Surface className="rounded-lg p-4">
  ...
  <div className="mt-3">
    ...
    <Text className="text-devil-subtle">{description}</Text>
...
All those components (`Surface`, `Text`, `Button`) come from devil-ui. But the specific arrangement? That's a pattern you own and customize.
...
| **Ownership**     | devil-ui maintains           | You own & customize          |
...
- [Installation](/installation) — Get started with devil-ui
