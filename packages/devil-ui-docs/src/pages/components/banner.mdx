...
<CodeBlock code={`import { Banner } from "@hellwrk/devil-ui";`} lang="tsx" />

### Granular

  <CodeBlock
    code={`import { Banner } from "@hellwrk/devil-ui/components/banner";`}
    lang="tsx"
  />
</ComponentSection>
...
```tsx
import { Banner } from "@hellwrk/devil-ui";
...
Use `size="sm"` in dialogs and other space-constrained contexts. Here, a banner should support the surrounding experience without competing with its primary action, so prefer a devil-ui `Link` that renders inline with the description. When greater emphasis is warranted, use `Banner.Action`, which renders at the trailing end.
