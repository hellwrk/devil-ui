...
title: "Contributing"
description: "Learn how to contribute to devil-ui, from setup and workflows to PR and release guidelines. Everything you need for setup, development, quality checks, and PR process is documented here."
---
...
Internal contributors normally clone and push directly to `hellwrk/devil-ui`:
...
If you do not have write access, contact your manager or devil-ui maintainers.
...
- **Component**: reusable UI primitive in `packages/devil-ui/src/components/`
- **Block**: installable composition pattern in `packages/devil-ui/src/blocks/`
- **Docs only**: docs/demo/navigation updates in `packages/devil-ui-docs/`
...
```bash
# Terminal 1: watch Devil package output
pnpm --filter @hellwrk/devil-ui dev
...
1. Build the feature in `packages/devil-ui/src/...`.
2. Add or update demos in `packages/devil-ui-docs/src/components/demos/`.
3. Add/update tests in `packages/devil-ui`.
4. Add or update docs pages in `packages/devil-ui-docs/src/pages/`.
...
- Follow existing devil-ui patterns for variants, props, and structure.
...
```bash
pnpm format
pnpm --filter @hellwrk/devil-ui test:run
```
...
If your change touches `packages/devil-ui/` and should ship, add a changeset:
...
- Use PR title format: `[package] short description` (example: `[devil] add meter warning variant`).
...
- Most devil-ui tests are Vitest-based and live under `packages/devil-ui`.
...
- Use semantic devil-ui tokens and avoid raw Tailwind color classes.
...
- Do not skip changesets for releasable `packages/devil-ui` changes.

---

## Related Docs

Follow the [changeset guide](https://github.com/hellwrk/devil-ui/blob/main/.changeset/README.md) to document releasable library changes and see the [AGENTS.md](https://github.com/hellwrk/devil-ui/blob/main/packages/devil-ui/AGENTS.md) for conventions and patterns.
