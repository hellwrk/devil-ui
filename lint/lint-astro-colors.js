 ...
 * 2. Invalid/unknown semantic tokens not defined in theme-devil.css
 *
 * Usage:
...
const VALID_DEVIL_SEMANTIC_COLORS = new Set([
  ...VALID_COLOR_TOKENS,
  ...
  for (const { value, line } of classAttrs) {
    ...
    if (primitive) {
      issues.push({
        ...
        message: `Avoid Tailwind primitive color '${primitive.token}'. Use Devil semantic tokens instead.`,
      });
      continue;
    }
    ...
    if (invalid) {
      issues.push({
        ...
        message: `Invalid color token '${invalid.token}'. Token '${invalid.tokenName}' is not defined in theme-devil.css.`,
      });
    }
  }
...
function main() {
  ...
  for (const { path, issues } of fileIssues) {
    for (const issue of issues) {
      ...
      console.log(
        `  ${symbol} devil/${issue.type === "primitive" ? "no-primitive-colors" : "invalid-color-token"}: ${issue.message}`,
      );
