...
// We want to enforce use of Devil semantic color tokens `--color-devil-*`.
// Any Tailwind color utility (e.g. `bg-blue-500`) or legacy semantic
// utility (e.g. `bg-devil-hairline`, `text-devil-default`) in class strings should be
...
function parseDevilSemanticColors() {
  const themeFiles = [
    resolve(__dirname, "../packages/devil-ui/src/styles/theme-devil.css"),
    resolve(__dirname, "../packages/devil-ui/src/styles/theme-fedramp.css"),
  ];
...
// Valid Devil semantic color tokens derived from theme CSS files.
...
} = parseDevilSemanticColors();

// Legacy export for backwards compatibility
export const VALID_DEVIL_SEMANTIC_COLORS = new Set([
  ...VALID_COLOR_TOKENS,
...
function findPrimitiveColor(str) {
  ...
  while ((match = TOKEN_RE.exec(str))) {
    ...
    // Skip valid Devil semantic color tokens (e.g. bg-surface, text-secondary,
    // border-devil-fill). These are backed by theme-devil.css custom properties.
    if (VALID_DEVIL_SEMANTIC_COLORS.has(colorFamily)) continue;

    // Flag devil- prefixed classes (e.g. text-devil-surface).
    // These are invalid; use semantic tokens directly (e.g. text-devil-default).
    if (colorFamily.startsWith("devil-"))
    ...
    // Only flag if it's a Tailwind primitive AND not a valid Devil semantic token.
    // This handles cases like "green-2" where "green" is a Tailwind primitive
    // but "green-2" is a valid Devil semantic token.
    if (
      TAILWIND_COLOR_FAMILIES.has(primitiveFamily) &&
      !VALID_DEVIL_SEMANTIC_COLORS.has(colorFamily)
    )
      return { type: "primitive", token: fullToken };
  }
...
export const noPrimitiveColorsRule = defineRule({
  meta: {
    ...
    messages: {
      [RULE_NAME]:
        "Avoid Tailwind color utilities (e.g. `bg-blue-500`, `border-red-500`). Use Devil semantic tokens instead.",
      [INVALID_TOKEN_RULE]:
        "Invalid color token '{{token}}'. Token '{{tokenName}}' is not defined in theme-devil.css or theme-fedramp.css.",
    },
    schema: [],
  },
  defaultOptions: [],
