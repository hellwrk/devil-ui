...
# Devil Docs Preview Deployment Script
# Uploads a new version to devil-docs worker, outputs report artifact
#
# PREREQUISITES:
# - Preview URLs must be enabled in Cloudflare dashboard:
#   Workers & Pages > devil-docs > Settings > Domains & Routes > Preview URLs > Enable
# - wrangler.jsonc must have "preview_urls": true (already configured)

echo "📚 Starting devil-docs preview deployment..."
...
export DEVIL_DOCS_PREVIEW_URL="$PREVIEW_URL"
echo "✅ Devil docs preview available: $DEVIL_DOCS_PREVIEW_URL"

# Output report artifact for the PR reporter job
echo "📄 Writing report artifact..."
pnpm tsx ci/scripts/write-devil-docs-report.ts

echo "🎉 Devil docs preview deployment complete!"
