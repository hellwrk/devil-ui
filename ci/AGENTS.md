...
│   ├── devil-docs-preview.ts   # Docs preview URL (priority 30)
│   └── visual-regression.ts   # Screenshot diff report
├── scripts/
│   ├── validate-devil-changeset.ts   # Pre-push + CI changeset enforcement
│   ├── ensure-changeset-config.ts   # Guard: .changeset/config.json exists
│   ├── write-npm-report.ts          # Writes ci/reports/npm-release.json
│   ├── write-devil-docs-report.ts    # Writes ci/reports/devil-docs-preview.json
...
│   ├── github-api.ts          # Octokit wrapper (hardcoded: hellwrk/devil-ui)
...
| Task                 | Location                                     | Notes                                                   |
| -------------------- | -------------------------------------------- | ------------------------------------------------------- |
| Changeset validation | `scripts/validate-devil-changeset.ts`         | Used by `.vite-hooks/pre-push` AND CI                   |
...
deploy-docs-preview.sh → write-devil-docs-report.ts → ci/reports/devil-docs-preview.json ─┘
```
...
- Data channel: environment variables (`PACKAGE_VERSION`, `DEVIL_DOCS_PREVIEW_URL`, `GITHUB_*`)
