...
export interface ReportItem {
  /** Unique identifier for this report type (e.g., "npm-release", "devil-docs-preview") */
  id: string;
...
export interface CIContext {
  ...
  /** Devil docs preview URL (if deployed) */
  devilDocsPreviewUrl?: string;
...
export function buildContextFromEnv(): CIContext {
  ...
  return {
    ...
    repository: process.env.GITHUB_REPOSITORY ?? "hellwrk/devil-ui",
    apiToken: process.env.GITHUB_TOKEN ?? "",
    packageName: process.env.PACKAGE_NAME ?? "@hellwrk/devil-ui",
    packageVersion: process.env.PACKAGE_VERSION ?? "",
    devilDocsPreviewUrl: process.env.DEVIL_DOCS_PREVIEW_URL,
  };
}
