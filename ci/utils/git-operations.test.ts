...
function createRepositoryWithStaleMergeRef(): {
  ...
  const directory = mkdtempSync(join(tmpdir(), "devil-git-operations-"));
  ...
  writeFile(repository, "packages/devil-ui/src/new-on-main.ts", "export {};\n");
  writeFile(
    repository,
    ".changeset/new-on-main.md",
    '---\n"@hellwrk/devil-ui": patch\n---\n\nMain change.\n',
  );
  git(repository, "add", "packages/devil-ui/src/new-on-main.ts");
...
describe("pull request diffs", () => {
  it("does not attribute newer base-branch changes to a stale merge ref", () => {
    ...
    try {
      ...
      assert.deepEqual(
        getChangedFiles({ cwd: repository, filterPath: "packages/devil-ui" }),
        [],
      );
      assert.deepEqual(
