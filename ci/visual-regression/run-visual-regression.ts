...
function generateMarkdownReport(comparisons: ComparisonResult[]): string {
  ...
  lines.push("");
  lines.push("---");
  lines.push("*Generated by Devil Visual Regression*");
  lines.push("</details>");
...
async function postPRComment(body: string): Promise<void> {
  ...
  const repo = process.env.GITHUB_REPOSITORY ?? "hellwrk/devil-ui";
  ...
  const marker = "<!-- devil-visual-regression -->";
