...
```bash
# Query component API
jq '.components.Button' packages/devil-ui/ai/component-registry.json

# Or use the CLI
pnpm --filter @hellwrk/devil-ui doc Button
```

The registry at `packages/devil-ui/ai/component-registry.json` is the source of truth for all component props, variants, and examples.
...
| Do                                       | Do Not                             |
| ---------------------------------------- | ---------------------------------- |
| `bg-devil-base`, `bg-devil-elevated`       | `bg-gray-100`, `bg-slate-50`       |
| `text-devil-default`, `text-devil-subtle`  | `text-gray-900`, `text-gray-500`   |
| `border-devil-line`, `ring-devil-hairline` | `border-gray-200`, `ring-blue-500` |
| `bg-devil-brand`, `bg-devil-danger`        | `bg-orange-500`, `bg-red-500`      |
...
1. Export `DEVIL_{NAME}_VARIANTS` and `DEVIL_{NAME}_DEFAULT_VARIANTS`
...
| File                         | Edit Instead                                                       |
| ---------------------------- | ------------------------------------------------------------------ |
| `src/styles/theme-devil.css`  | `scripts/theme-generator/config.ts`                                |
...
pnpm --filter @hellwrk/devil-ui test         # Vitest (happy-dom env, v8 coverage)
pnpm --filter @hellwrk/devil-ui codegen:registry  # Regenerate registry (auto-runs in build)
...
Any change to `packages/devil-ui/` requires a changeset:
...
The repo uses oxlint with 5 custom JS rules in `lint/` (shared) and `packages/devil-ui/lint/` (adds `no-deprecated-props`).
...
| Pattern                      | Problem                    | Fix                         |
| ---------------------------- | -------------------------- | --------------------------- |
| `bg-blue-500`                | Breaks theming, fails lint | `bg-devil-brand`             |
